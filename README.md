# nusoma — Command Line User Interface for Claude Code

A lightweight, transparent desktop overlay for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) on macOS. nusoma wraps the Claude Code CLI in a floating pill interface with multi-tab sessions, a permission approval UI, voice input, and a skills marketplace.
...
- **Skills marketplace** — install plugins from Anthropic's GitHub repos without leaving nusoma.
...
The fastest way to get nusoma running as a regular Mac app. This installs dependencies, voice support (Whisper), builds the app, copies it to `/Applications`, and launches it.

**1) Clone the repo**

```bash
git clone https://github.com/lcoutodemos/nusoma-cc.git
```

**2) Double-click `install-app.command`**

Open the `nusoma-cc` folder in Finder and double-click `install-app.command`.
...
<p align="center"><img src="docs/shortcut.png" width="520" alt="Press Option + Space to show or hide nusoma" /></p>

After the initial install, just open **nusoma** from your Applications folder or Spotlight.
...
| `./commands/stop.command` | Stop all nusoma processes |
...
├── preload/                # Secure IPC bridge (window.nusoma API)
...
nusoma operates almost entirely offline. The only outbound network calls are:
...
- **Requires Claude Code CLI** — nusoma is a UI layer, not a standalone AI client. You need an authenticated `claude` CLI.
