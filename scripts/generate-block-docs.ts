...
// Paths configuration
const BLOCKS_PATH = path.join(rootDir, 'apps/nusoma/blocks/blocks')
const DOCS_OUTPUT_PATH = path.join(rootDir, 'apps/docs/content/docs/tools')
const ICONS_PATH = path.join(rootDir, 'apps/nusoma/components/icons.tsx')
  ...
  if (exportMatch) {
    ...
    if (blockStartMatch) {
      ...
      while (blockEnd < afterExport.length && braceCount > 0) {
        if (afterExport[blockEnd] === '{') {
          braceCount++
        } else if (afterExport[blockEnd] === '}') {
          braceCount--
        }
        blockEnd++
      }
  ...
  try {
    ...
    if (params.length === 0) {
      ...
      while ((match = paramRegex.exec(fileContent)) !== null) {
        ...
        if (match[1] === 'params' || match[1] === 'tools') {
          continue
        }
...
async function getToolInfo(toolName: string): Promise<{
  ...
  try {
    ...
    for (let i = parts.length - 1; i >= 1; i--) {
      ...
      const toolDirPath = path.join(rootDir, `apps/nusoma/tools/${possiblePrefix}`)
    ...
    possibleLocations.push(path.join(rootDir, `apps/nusoma/tools/${toolPrefix}/${toolSuffix}.ts`))
    ...
    if (toolSuffix.includes('_')) {
      ...
      possibleLocations.push(
        path.join(rootDir, `apps/nusoma/tools/${toolPrefix}/${underscoreSuffix}.ts`)
      )
    }
    ...
    const camelCaseSuffix = toolSuffix
      ...
      .join('')
    possibleLocations.push(
      path.join(rootDir, `apps/nusoma/tools/${toolPrefix}/${camelCaseSuffix}.ts`)
    )

    // Also check the index.ts file in the tool directory
    possibleLocations.push(path.join(rootDir, `apps/nusoma/tools/${toolPrefix}/index.ts`))
...
function mergeWithManualContent(
  ...
  Object.keys(manualSections).forEach((_section) => {})
...
function generateMarkdownTable(outputs: Record<string, string>): string {
  ...
  for (const [key, value] of Object.entries(outputs)) {
    ...
    if (value.toLowerCase().includes('array')) {
      inferredType = 'array'
    }
    ...
    if (value.toLowerCase().includes('boolean')) {
