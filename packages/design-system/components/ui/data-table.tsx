...
import { type Column, flexRender, type Table as ReactTable, type Row } from '@tanstack/react-table'
import {
  ArrowDownIcon,
  ArrowUpDownIcon,
  ArrowUpIcon,
...
function DataTableColumnOptionsHeader<TData>({ table }: DataTableColumnOptionsHeaderProps<TData>) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
...
function DataTableBulkActions<TData>({
  ...
  return (
    <div className='absolute inset-x-0 bottom-12 z-50 mx-auto flex h-[60px] max-w-xl animate-fadeIn items-center justify-between rounded-md border bg-background px-6 py-3 shadow'>
      <p className='font-semibold text-sm'>{table.getSelectedRowModel().rows.length} selected</p>
      {children}
    </div>
  )
}
...
function DataTableFilter({ title, options, selected, onChange }: DataTableFilterProps) {
