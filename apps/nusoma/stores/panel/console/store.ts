...
const MAX_ENTRIES = 50 // MAX across all workers
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        clearConsole: (workerId: string | null) => {
          set((state) => ({
            entries: workerId ? state.entries.filter((entry) => entry.workerId !== workerId) : [],
          }))
        },

        getWorkerEntries: (workerId) => {
          return get().entries.filter((entry) => entry.workerId === workerId)
        },
