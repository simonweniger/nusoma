...
describe('Console Store', () => {
  ...
  describe('addConsole', () => {
    it('should add a new console entry with required fields', () => {
      ...
      expect(newEntry.workerId).toBe('worker-123')
  ...
  describe('clearConsole', () => {
    beforeEach(() => {
      ...
      // Add multiple entries for different workers
      store.addConsole({
        workerId: 'worker-1',
        blockId: 'block-1',
        blockName: 'Block 1',
    ...
    it('should clear all entries when workerId is null', () => {
      const store = useConsoleStore.getState()
    ...
    it('should clear only specific worker entries', () => {
      const store = useConsoleStore.getState()

      expect(store.entries).toHaveLength(2)

      store.clearConsole('worker-1')

      const state = useConsoleStore.getState()
      expect(state.entries).toHaveLength(1)
      expect(state.entries[0].workerId).toBe('worker-2')
    })
  })

  describe('getWorkerEntries', () => {
    beforeEach(() => {
      const store = useConsoleStore.getState()

      // Add entries for different workers
      store.addConsole({
        workerId: 'worker-1',
        blockId: 'block-1',
        blockName: 'Block 1',
    ...
    it('should return entries for specific worker', () => {
      const store = useConsoleStore.getState()

      const worker1Entries = store.getWorkerEntries('worker-1')
      const worker2Entries = store.getWorkerEntries('worker-2')

      expect(worker1Entries).toHaveLength(2)
      expect(worker2Entries).toHaveLength(1)

      expect(worker1Entries.every((entry) => entry.workerId === 'worker-1')).toBe(true)
      expect(worker2Entries.every((entry) => entry.workerId === 'worker-2')).toBe(true)
    })

    it('should return empty array for non-existent worker', () => {
      const store = useConsoleStore.getState()

      const entries = store.getWorkerEntries('non-existent-worker')
