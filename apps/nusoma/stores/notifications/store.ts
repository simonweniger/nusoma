...
const STORAGE_KEY = 'worker-notifications'
// Maximum number of notifications to keep across all workers
...
export const useNotificationStore = create<NotificationStore>()(
  devtools(
    (set, get) => ({
      ...
      addNotification: (type, message, workerId, options: NotificationOptions = {}) => {
        ...
        set((state) => {
          ...
          const workerVisibleCount = get().getVisibleNotificationCount(workerId)

          if (workerVisibleCount > MAX_VISIBLE_NOTIFICATIONS) {
            // Find the oldest non-persistent visible notification from this worker to fade out
            newNotifications = newNotifications.map((n, index) => {
              ...
              // Only target notifications from the same worker that are visible, not persistent, and not already fading
              if (
                n.workerId === workerId &&
                n.isVisible &&
      ...
      markAllAsRead: (workerId) =>
        set((state) => {
          const newNotifications = state.notifications.map((n) =>
            n.workerId === workerId ? { ...n, read: true } : n
          )
          persistNotifications(newNotifications)
          return { notifications: newNotifications }
        }),
      ...
      getWorkerNotifications: (workerId) => {
        return get().notifications.filter((n) => n.workerId === workerId)
      },

      getVisibleNotificationCount: (workerId) => {
        if (!workerId) {
          return 0
        }

        return get().notifications.filter((n) => n.workerId === workerId && n.isVisible && !n.read)
          .length
      },
    }),
