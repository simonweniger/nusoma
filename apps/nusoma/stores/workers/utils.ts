...
import type { BlockState, SubBlockState } from './worker/types'

/**
 * Merges worker block states with subblock values while maintaining block structure
 * @param blocks - Block configurations from worker store
 * @param workerId - ID of the worker to merge values for
...
export function mergeSubblockState(
  ...
  // Get all the values stored in the subblock store for this worker
  const workerSubblockValues = workerId ? subBlockStore.workerValues[workerId] || {} : {}
  ...
  return Object.entries(blocksToProcess).reduce(
    (acc, [id, block]) => {
      ...
      const blockValues = workerSubblockValues[id] || {}
      ...
      const mergedSubBlocks = Object.entries(blockSubBlocks).reduce(
        (subAcc, [subBlockId, subBlock]) => {
          ...
          // If workerId is provided, use it to get the value
          if (workerId) {
            // Try to get the value from the subblock store for this specific worker
            if (blockValues[subBlockId] !== undefined) {
              storedValue = blockValues[subBlockId]
            }
          } else {
            // Fall back to the active worker if no workerId is provided
...
/**
 * Asynchronously merges worker block states with subblock values
      ...
      const subBlockEntries = await Promise.all(
        Object.entries(block.subBlocks).map(async ([subBlockId, subBlock]) => {
          ...
          if (workerId) {
            ...
            if (workerValues?.[id]) {
              storedValue = workerValues[id][subBlockId]
            }
          } else {
            // Fall back to the active worker if no workerId is provided
            storedValue = subBlockStore.getValue(id, subBlockId)
