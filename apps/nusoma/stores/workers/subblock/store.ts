...
const syncDebounceTimer: NodeJS.Timeout | null = null
const DEBOUNCE_DELAY = 500 // 500ms delay for sync

/**
 * SubBlockState stores values for all subblocks in workers
 *
 * Important implementation notes:
 * 1. Values are stored per worker, per block, per subblock
 * 2. When workers are synced to the database, the mergeSubblockState function
...
export const useSubBlockStore = create<SubBlockStore>()(
  devtools((set, get) => ({
      ...
      set((state) => ({
        workerValues: {
          ...
          [activeWorkerId]: {
            ...
            [blockId]: {
              ...state.workerValues[activeWorkerId]?.[blockId],
              [subBlockId]: value,
            },
          },
        },
    ...
    getValue: (blockId: string, subBlockId: string) => {
      ...
      return get().workerValues[activeWorkerId]?.[blockId]?.[subBlockId] ?? null
    },
    ...
    initializeFromWorker: (workerId: string, blocks: Record<string, any>) => {
