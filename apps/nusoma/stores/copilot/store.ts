        ...
        try {
          ...
          const response = await fetch('/api/copilot', {
            ...
            body: JSON.stringify({
              ...
              workerState: {
                blocks: workerStore.blocks,
                edges: workerStore.edges,
              },
            }),
          })
          ...
          if (data.actions) {
            ...
            blockActions.forEach((action: any, index: number) => {
              ...
              const position = calculateBlockPosition(workerStore.blocks, index)

              // Generate name if not provided
              const blockName = name || `${type} ${getNextBlockNumber(workerStore.blocks, type)}`

              workerStore.addBlock(id, type, blockName, position)
            })
            ...
            otherActions.forEach((action: any) => {
              switch (action.name) {
                ...
                case 'removeBlock': {
                  workerStore.removeBlock(action.parameters.id)
                  break
                }
                case 'removeEdge': {
                  workerStore.removeEdge(action.parameters.id)
                  break
                }
              }
            })
