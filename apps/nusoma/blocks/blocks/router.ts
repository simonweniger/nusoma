import { ConnectIcon } from '@nusoma/design-system/components/icons'
...
export const generateRouterPrompt = (prompt: string, targetBlocks?: TargetBlock[]): string => {
  const basePrompt = `You are an intelligent routing agent responsible for directing worker requests to the most appropriate block. Your task is to analyze the input and determine the single most suitable destination based on the request.

Key Instructions:
1. You MUST choose exactly ONE destination from the IDs of the blocks in the worker. The destination must be a valid block id.
...
export const RouterBlock: BlockConfig<RouterResponse> = {
  ...
  longDescription:
    'Intelligently direct worker execution to different paths based on input analysis. Use AI to determine the most appropriate next step in your worker based on content, intent, or specific criteria.',
  docsLink: 'https://docs.nusoma.ai/blocks/router',
  category: 'blocks',
  bgColor: '#28C43F',
  icon: ConnectIcon,
  subBlocks: [
