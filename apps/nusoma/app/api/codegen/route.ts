...
2.  Reference Input Parameters/Worker Variables: Use the exact syntax <variable_name>. Do NOT wrap it in quotes (e.g., use 'userId = <userId>;' not 'userId = "<userId>";'). This includes parameters defined in the block's schema and outputs from previous blocks.
3.  Function Body ONLY: Do NOT include the function signature (e.g., 'async function myFunction() {' or the surrounding '}').
4.  Imports: Do NOT include import/require statements unless they are standard Node built-in modules (e.g., 'crypto', 'fs'). External libraries are not supported in this context.
...
try {
  ...
  // Re-throwing the error ensures the worker knows this step failed.
  throw error;
}`,
...
Do not include import/require statements unless absolutely necessary and they are standard Node modules.
  ...
  try {
    ...
    if (!prompt || !generationType) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required fields: prompt and generationType.',
        },
        { status: 400 }
      )
    }
    ...
    if (stream) {
      try {
        ...
        return NextResponse.json(
          {
            success: false,
            error: 'An error occurred during code generation streaming.',
          },
          { status: 500 }
        )
      }
    ...
    if (!generatedContent) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to generate content. OpenAI response was empty.',
        },
        { status: 500 }
      )
    }

    logger.info(`[${requestId}] Code generation successful`, {
      generationType,
    })
