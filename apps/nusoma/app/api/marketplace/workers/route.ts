...
import { CATEGORIES } from '@/app/(dashboard)/marketplace/constants/categories'
import { createErrorResponse, createSuccessResponse } from '@/app/api/workers/utils'

const logger = createLogger('MarketplaceWorkersAPI')

// Cache for 1 minute but can be revalidated on-demand
export const revalidate = 60

/**
 * Consolidated API endpoint for marketplace workers
 *
 * Supports:
 * - Getting featured/popular/recent workers
 * - Getting workers by category
 * - Getting worker state
 ...
 * - includeState: Whether to include worker state in the response (default: false)
 * - workerId: Specific worker ID to fetch details for
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Handle single worker request first (by worker ID)
    if (workerId) {
      ...
      if (includeState) {
        ...
        marketplaceEntry = await db
          ...
          .where(eq(schema.marketplace.workerId, workerId))
          .limit(1)
          .then((rows) => rows[0])
      } else {
        // Query without state
      ...
      if (!marketplaceEntry) {
        logger.warn(`[${requestId}] No marketplace entry found for worker: ${workerId}`)
        return createErrorResponse('Worker not found in marketplace', 404)
      }
      ...
      const responseData =
        includeState && 'state' in marketplaceEntry
          ? {
            ...marketplaceEntry,
            workerState: marketplaceEntry.state,
            state: undefined,
          }
          : marketplaceEntry

      logger.info(`[${requestId}] Retrieved marketplace data for worker: ${workerId}`)
    ...
    if (includeState) {
      const transformSection = (section: any[]) => {
        return section.map((item) => {
          if ('state' in item) {
            // Create a new object without the state field, but with workerState
