 ...
 * - workerId: Filter by worker ID (required)
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Extract workerId from query parameters
    ...
    // Filter by worker ID (required)
    conditions.push(eq(memory.workerId, workerId))
    ...
    logger.info(`[${requestId}] Found ${memories.length} memories for worker: ${workerId}`)
    return NextResponse.json(
      {
        success: true,
        data: { memories },
 ...
 * Requires:
 * - key: Unique identifier for the memory (within worker scope)
 * - type: Memory type ('agent')
 * - data: Memory content (agent message with role and content)
 * - workerId: ID of the worker this memory belongs to
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { key, type, data, workerId } = body
    ...
    // Check if memory with the same key already exists for this worker
    const existingMemory = await db
      .select()
      .from(memory)
      .where(and(eq(memory.key, key), eq(memory.workerId, workerId), isNull(memory.deletedAt)))
      .limit(1)
    ...
    if (existingMemory.length > 0) {
      ...
      await db
        ...
        .where(and(eq(memory.key, key), eq(memory.workerId, workerId)))
      ...
      logger.info(`[${requestId}] Memory created successfully: ${key} for worker: ${workerId}`)
    }

    // Fetch all memories with the same key for this worker to return the complete list
    ...
    logger.info(`[${requestId}] Memory operation successful: ${key} for worker: ${workerId}`)
    return NextResponse.json(
      {
        success: true,
        data: memoryRecord,
