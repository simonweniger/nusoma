...
export interface CapturedWorkerValues {
  name?: string
  description?: string
  color?: string
...
export const sampleWorkerState = {
  blocks: {
    'starter-id': {
      id: 'starter-id',
      type: 'starter',
...
export function mockWorkerAccessValidation(shouldSucceed = true) {
  if (shouldSucceed) {
    vi.mock('@/app/api/workers/middleware', () => ({
...
export async function getMockedDependencies() {
  ...
  const logsModule = await import('@/lib/logger/execution-logger')
  const traceSpansModule = await import('@/lib/logger/trace-spans')
  const workerUtilsModule = await import('@/lib/workers/utils')
  const executorModule = await import('@/executor')
  const serializerModule = await import('@/serializer')
  const dbModule = await import('@nusoma/database')
  ...
  return {
    ...
    updateWorkerRunCounts: workerUtilsModule.updateWorkerRunCounts,
  ...
  vi.doMock('@/db', () => {
    const select = vi.fn().mockImplementation(() => ({
      from: vi.fn().mockImplementation((table: any) => {
        ...
        if (tbl === 'worker_schedule' || tbl === 'schedule') {
          return {
            where: vi.fn().mockImplementation(() => ({
        ...
        if (tbl === 'worker') {
          return {
            where: vi.fn().mockImplementation(() => ({
              limit: vi.fn().mockImplementation(() => [workerRecord]),
            })),
          }
        }
...
export function mockSupabase() {
  ...
  vi.doMock('@nusoma/database/supabase', () => ({
...
export function setupComprehensiveTestMocks(options: TestSetupOptions = {}) {
  ...
  if (features.workerUtils) {
    featureMocks.workerUtils = mockWorkerUtils()
  }
...
/**
 * Mock worker utilities and response helpers
 */
export function mockWorkerUtils() {
  vi.doMock('@/app/api/workers/utils', () => ({
