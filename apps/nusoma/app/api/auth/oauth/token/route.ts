 ...
 * and worker-based authentication (for server-side requests)
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { credentialId, workerId } = body
    ...
    if (!userId) {
      return NextResponse.json(
        { error: workerId ? 'Worker not found' : 'User not authenticated' },
        { status: workerId ? 404 : 401 }
      )
    }
