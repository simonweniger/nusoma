...
import { account, worker } from '@nusoma/database/schema'
...
/**
 * Get the user ID based on either a session or a worker ID
 */
export async function getUserId(requestId: string, workerId?: string): Promise<string | undefined> {
  // If workerId is provided, this is a server-side request
  if (workerId) {
    // Get the worker to verify the user ID
    ...
    if (!workers.length) {
      logger.warn(`[${requestId}] Worker not found`)
      return undefined
    }

    return workers[0].userId
  }
