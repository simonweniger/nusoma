...
describe('OAuth Utils', () => {
  ...
  describe('getUserId', () => {
    it('should get user ID from session when no workerId is provided', async () => {
    ...
    it('should get user ID from worker when workerId is provided', async () => {
      mockDb.limit.mockReturnValueOnce([{ userId: 'worker-owner-id' }])

      const { getUserId } = await import('./utils')

      const userId = await getUserId('request-id', 'worker-id')

      expect(mockDb.select).toHaveBeenCalled()
      expect(mockDb.from).toHaveBeenCalled()
      expect(mockDb.where).toHaveBeenCalled()
      expect(mockDb.limit).toHaveBeenCalledWith(1)
      expect(userId).toBe('worker-owner-id')
    })
    ...
    it('should return undefined if worker is not found', async () => {
      mockDb.limit.mockReturnValueOnce([])

      const { getUserId } = await import('./utils')

      const userId = await getUserId('request-id', 'nonexistent-worker-id')
