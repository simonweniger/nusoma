...
describe('Schedule Configuration API Route', () => {
  beforeEach(() => {
    ...
    // Extend sampleWorkerState for scheduling
    const _workerStateWithSchedule = {
      ...sampleWorkerState,
      blocks: {
        ...sampleWorkerState.blocks,
        'starter-id': {
          ...sampleWorkerState.blocks['starter-id'],
          subBlocks: {
            ...sampleWorkerState.blocks['starter-id'].subBlocks,
            startWorker: { id: 'startWorker', type: 'dropdown', value: 'schedule' },
   ...
   */
  it('should remove a schedule when startWorker is not schedule', async () => {
  ...
  vi.doMock('@nusoma/database', () => ({
    ...
    vi.doMock('@/lib/schedules/utils', () => ({
      ...
      getSubBlockValue: vi.fn().mockImplementation((block: any, id: string) => {
        const subBlocks = {
          startWorker: 'manual', // Changed to manual
          scheduleType: 'daily',
        }
        return subBlocks[id] || ''
      }),
      ...
      state: { 
        blocks: {
          'starter-id': {
            type: 'starter',
            subBlocks: {
              startWorker: { value: 'manual' }, // Manual trigger
              scheduleType: { value: 'daily' },
            }
          }
        },
