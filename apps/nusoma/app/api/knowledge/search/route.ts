...
import { embedding, knowledgeBase } from '@nusoma/database/schema'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { workerId, ...searchParams } = body

    const userId = await getUserId(requestId, workerId)

    if (!userId) {
      const errorMessage = workerId ? 'Worker not found' : 'Unauthorized'
      const statusCode = workerId ? 404 : 401
