...
describe('Knowledge Search API Route', () => {
  ...
  describe('POST /api/knowledge/search', () => {
    ...
    it('should handle worker-based authentication', async () => {
      const workerData = {
        ...validSearchData,
        workerId: 'worker-123',
      }
      ...
      const req = createMockRequest('POST', workerData)
      const { POST } = await import('./route')
      const response = await POST(req)
      const data = await response.json()
      ...
      expect(response.status).toBe(200)
      expect(data.success).toBe(true)
      expect(mockGetUserId).toHaveBeenCalledWith(expect.any(String), 'worker-123')
    })
    ...
    it('should return not found for worker that does not exist', async () => {
      const workerData = {
        ...validSearchData,
