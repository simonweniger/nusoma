...
import { customTools } from '@nusoma/database/schema'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // If workerId is provided, get userId from the worker
    if (workerId) {
      userId = await getUserId(requestId, workerId)

      if (!userId) {
        logger.warn(`[${requestId}] No valid user found for worker: ${workerId}`)
