    ...
    try {
      ...
      const ensureLoginPage = async (): Promise<boolean> => {
        ...
        try {
          ...
          // Try direct navigation to /auth/sign-in if we couldn't find login elements
          logger.info('Trying direct navigation to /auth/sign-in path')
          const currentUrl = await stagehand.page.url()
          const loginUrl = new URL('/auth/sign-in', currentUrl).toString()
