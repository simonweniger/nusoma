...
export async function POST(request: Request) {
  try {
    ...
    const { credential, teamId, workerId } = body
    ...
    try {
      // Get the userId either from the session or from the workerId
      ...
      const accessToken = await refreshAccessTokenIfNeeded(credential, userId, workerId)
