...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.debug(`[${requestId}] Fetching webhooks for user ${session.user.id}`, {
      filteredByWorker: !!workerId,
    })

    // Create where condition
    const whereCondition = workerId
      ? and(eq(worker.userId, session.user.id), eq(webhook.workerId, workerId))
      : eq(worker.userId, session.user.id)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { workerId, path, provider, providerConfig } = body
    ...
    const workers = await db
      .select({ id: worker.id }) // Select only necessary field
      .from(worker)
      .where(and(eq(worker.id, workerId), eq(worker.userId, userId)))
      .limit(1)

    if (workers.length === 0) {
      logger.warn(`[${requestId}] Worker not found or not owned by user: ${workerId}`)
      return NextResponse.json({ error: 'Worker not found' }, { status: 404 })
    }

    // Check if a webhook with the same path already exists
    const existingWebhooks = await db
      .select({ id: webhook.id, workerId: webhook.workerId })
      .from(webhook)
    ...
    // If a webhook with the same path exists but belongs to a different worker, return an error
    if (existingWebhooks.length > 0 && existingWebhooks[0].workerId !== workerId) {
    ...
    // If a webhook with the same path and workerId exists, update it
    if (existingWebhooks.length > 0 && existingWebhooks[0].workerId === workerId) {
