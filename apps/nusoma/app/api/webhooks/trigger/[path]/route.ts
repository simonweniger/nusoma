...
import { webhook, worker } from '@nusoma/database/schema'
...
export async function POST(
  ...
  // Find webhook and associated worker
  const webhooks = await db
    .select({
      webhook: webhook,
      worker: worker,
    ...
    .from(webhook)
    .innerJoin(worker, eq(webhook.workerId, worker.id))
    .where(and(eq(webhook.path, path), eq(webhook.isActive, true)))
    .limit(1)
  ...
  foundWorker = webhooks[0].worker

  const normalizedData = await loadWorkerFromNormalizedTables(foundWorker.id)

  if (!normalizedData) {
    logger.error(`[${requestId}] No normalized data found for webhook worker ${foundWorker.id}`)
    return new NextResponse('Worker data not found in normalized tables', { status: 500 })
  }
  ...
  foundWorker.state = {
    ...
    lastSaved: Date.now(),
    isDeployed: foundWorker.isDeployed || false,
    deployedAt: foundWorker.deployedAt,
  }
  ...
  const processingPromise = (async () => {
    try {
      ...
      const usageCheck = await checkServerSideUsageLimits(foundWorker.userId)
      if (usageCheck.isExceeded) {
        logger.warn(
          `[${requestId}] User ${foundWorker.userId} has exceeded usage limits. Skipping webhook execution.`,
          {
        ...
        // Return a successful response to avoid webhook retries, but don't execute the worker
      ...
      // Execute worker for the webhook event
      logger.info(`[${requestId}] Executing worker for ${foundWebhook.provider} webhook`)

      const executionId = uuidv4()
      return await processWebhook(foundWebhook, foundWorker, body, request, executionId, requestId)
    } catch (error: any) {
