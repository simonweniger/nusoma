...
const webhookMock = {
  ...
  path: 'path-column',
  workerId: 'worker-id-column',
  isActive: 'is-active-column',
  provider: 'provider-column',
}
const workerMock = { id: 'worker-id-column' }
...
describe('Webhook Trigger API Route', () => {
  ...
  it('should handle WhatsApp verification challenge', async () => {
    ...
    const { db } = await import('@nusoma/database')
  ...
  /**
   * Test POST webhook with worker execution
   * Verifies that a webhook trigger properly initiates worker execution
   */
  it('should trigger worker execution via POST', async () => {
    ...
    // Configure DB mock to return a webhook and worker
    const { db } = await import('@nusoma/database')
    const limitMock = vi.fn().mockReturnValue([
      {
        webhook: {
   ...
   */
  it('should handle errors during worker execution', async () => {
