...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    ...
    return NextResponse.redirect(
      new URL(
        ...
        env.NEXT_PUBLIC_APP_URL || 'https://nusoma.ai'
      )
    )
  }
  ...
  if (!session?.user?.id) {
    ...
    return NextResponse.redirect(
      new URL(`/invite/${token}?token=${token}`, env.NEXT_PUBLIC_APP_URL || 'https://nusoma.ai')
    )
  }
  ...
  try {
    ...
    if (existingMembership) {
      ...
      return NextResponse.redirect(
        new URL(`/worker/${invitation.workspaceId}`, env.NEXT_PUBLIC_APP_URL || 'https://nusoma.ai')
      )
    }
