/**
 * Tests for worker variables API route
 * Tests the optimized permissions and caching system
 *
 * @vitest-environment node
 */

import { NextRequest } from 'next/server'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import {
  createMockDatabase,
  mockAuth,
  mockCryptoUuid,
  mockUser,
  setupCommonApiMocks,
} from '@/app/api/__test-utils__/utils'

describe('Worker Variables API Route', () => {
  let authMocks: ReturnType<typeof mockAuth>
  let databaseMocks: ReturnType<typeof createMockDatabase>

  beforeEach(() => {
    vi.resetModules()
    setupCommonApiMocks()
    mockCryptoUuid('mock-request-id-12345678')
    authMocks = mockAuth(mockUser)
  })

  afterEach(() => {
    vi.clearAllMocks()
  })

  describe('GET /api/workers/[id]/variables', () => {
    it('should return 401 when user is not authenticated', async () => {
      authMocks.setUnauthenticated()

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(401)
      const data = await response.json()
      expect(data.error).toBe('Unauthorized')
    })

    it('should return 404 when worker does not exist', async () => {
      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[]] }, // No worker found
      })

      const req = new NextRequest('http://localhost:3000/api/workers/nonexistent/variables')
      const params = Promise.resolve({ id: 'nonexistent' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(404)
      const data = await response.json()
      expect(data.error).toBe('Worker not found')
    })

    it('should allow access when user owns the worker', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'user-123',
        workspaceId: null,
        variables: {
          'var-1': { id: 'var-1', name: 'test', type: 'string', value: 'hello' },
        },
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(200)
      const data = await response.json()
      expect(data.data).toEqual(mockWorker.variables)
    })

    it('should allow access when user has workspace permissions', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'other-user',
        workspaceId: 'workspace-456',
        variables: {
          'var-1': { id: 'var-1', name: 'test', type: 'string', value: 'hello' },
        },
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      vi.doMock('@/lib/permissions/utils', () => ({
        getUserEntityPermissions: vi.fn().mockResolvedValue('read'),
      }))

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(200)
      const data = await response.json()
      expect(data.data).toEqual(mockWorker.variables)

      // Verify permissions check was called
      const { getUserEntityPermissions } = await import('@/lib/permissions/utils')
      expect(getUserEntityPermissions).toHaveBeenCalledWith(
        'user-123',
        'workspace',
        'workspace-456'
      )
    })

    it('should deny access when user has no workspace permissions', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'other-user',
        workspaceId: 'workspace-456',
        variables: {},
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      vi.doMock('@/lib/permissions/utils', () => ({
        getUserEntityPermissions: vi.fn().mockResolvedValue(null),
      }))

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(401)
      const data = await response.json()
      expect(data.error).toBe('Unauthorized')
    })

    it.concurrent('should include proper cache headers', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'user-123',
        workspaceId: null,
        variables: {
          'var-1': { id: 'var-1', name: 'test', type: 'string', value: 'hello' },
        },
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(200)
      expect(response.headers.get('Cache-Control')).toBe('max-age=30, stale-while-revalidate=300')
      expect(response.headers.get('ETag')).toMatch(/^"variables-worker-123-\d+"$/)
    })

    it.concurrent('should return empty object for workers with no variables', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'user-123',
        workspaceId: null,
        variables: null,
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(200)
      const data = await response.json()
      expect(data.data).toEqual({})
    })
  })

  describe('POST /api/workers/[id]/variables', () => {
    it('should allow owner to update variables', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'user-123',
        workspaceId: null,
        variables: {},
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
        update: { results: [{}] },
      })

      const variables = [
        { id: 'var-1', workerId: 'worker-123', name: 'test', type: 'string', value: 'hello' },
      ]

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables', {
        method: 'POST',
        body: JSON.stringify({ variables }),
      })
      const params = Promise.resolve({ id: 'worker-123' })

      const { POST } = await import('@/app/api/workers/[id]/variables/route')
      const response = await POST(req, { params })

      expect(response.status).toBe(200)
      const data = await response.json()
      expect(data.success).toBe(true)
    })

    it('should deny access for users without permissions', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'other-user',
        workspaceId: 'workspace-456',
        variables: {},
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      vi.doMock('@/lib/permissions/utils', () => ({
        getUserEntityPermissions: vi.fn().mockResolvedValue(null),
      }))

      const variables = [
        { id: 'var-1', workerId: 'worker-123', name: 'test', type: 'string', value: 'hello' },
      ]

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables', {
        method: 'POST',
        body: JSON.stringify({ variables }),
      })
      const params = Promise.resolve({ id: 'worker-123' })

      const { POST } = await import('@/app/api/workers/[id]/variables/route')
      const response = await POST(req, { params })

      expect(response.status).toBe(401)
      const data = await response.json()
      expect(data.error).toBe('Unauthorized')
    })

    it.concurrent('should validate request data schema', async () => {
      const mockWorker = {
        id: 'worker-123',
        userId: 'user-123',
        workspaceId: null,
        variables: {},
      }

      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { results: [[mockWorker]] },
      })

      // Invalid data - missing required fields
      const invalidData = { variables: [{ name: 'test' }] }

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables', {
        method: 'POST',
        body: JSON.stringify(invalidData),
      })
      const params = Promise.resolve({ id: 'worker-123' })

      const { POST } = await import('@/app/api/workers/[id]/variables/route')
      const response = await POST(req, { params })

      expect(response.status).toBe(400)
      const data = await response.json()
      expect(data.error).toBe('Invalid request data')
    })
  })

  describe('Error handling', () => {
    it.concurrent('should handle database errors gracefully', async () => {
      authMocks.setAuthenticated({ id: 'user-123', email: 'test@example.com' })
      databaseMocks = createMockDatabase({
        select: { throwError: true, errorMessage: 'Database connection failed' },
      })

      const req = new NextRequest('http://localhost:3000/api/workers/worker-123/variables')
      const params = Promise.resolve({ id: 'worker-123' })

      const { GET } = await import('@/app/api/workers/[id]/variables/route')
      const response = await GET(req, { params })

      expect(response.status).toBe(500)
      const data = await response.json()
      expect(data.error).toBe('Database connection failed')
    })
  })
})
