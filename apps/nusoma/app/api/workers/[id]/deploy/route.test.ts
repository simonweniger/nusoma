/**
 * Integration tests for worker deployment API route
 *
...
describe('Worker Deployment API Route', () => {
  beforeEach(() => {
    vi.resetModules()
   ...
   */
  it('should create new API key when deploying worker for user with no API key', async () => {
    ...
    const mockUpdate = vi.fn().mockReturnValue({
      set: vi.fn().mockReturnValue({
        where: vi.fn().mockResolvedValue([{ id: 'worker-id' }]),
      }),
    })
   ...
   */
  it('should use existing API key when deploying worker', async () => {
   ...
   */
  it('should undeploy worker successfully', async () => {
    const mockUpdate = vi.fn().mockReturnValue({
      set: vi.fn().mockReturnValue({
        where: vi.fn().mockResolvedValue([{ id: 'worker-id' }]),
      }),
   ...
   * Test error handling
   */
  it('should handle errors when worker is not found', async () => {
    vi.doMock('../../middleware', () => ({
      validateWorkerAccess: vi.fn().mockResolvedValue({
        error: {
   ...
   */
  it('should handle unauthorized access to worker', async () => {
    vi.doMock('../../middleware', () => ({
      validateWorkerAccess: vi.fn().mockResolvedValue({
        error: {
