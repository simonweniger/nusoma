...
import { AgentIcon } from '@nusoma/design-system/components/icons'
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function WorkspaceHeader({
  ...
  const { activeWorkspaceId, switchToWorkspace, setActiveWorkspaceId } = useWorkerRegistry()
      ...
      fetch('/api/workspaces')
        ...
        .then((data) => {
          if (data.workspaces && Array.isArray(data.workspaces)) {
            ...
            // If no activeWorkspaceId, let loadWorkspaceFromWorkerId handle workspace selection
          }
  ...
  const switchWorkspace = (workspace: Workspace) => {
    ...
    router.push(`/worker/${workspace.id}`)
  }
    ...
    fetch('/api/workspaces', {
      ...
      .then((data) => {
        if (data.workspace) {
          ...
          // Use switchToWorkspace to properly load workers for the new workspace
          // This will clear existing workers, set loading state, and fetch workers from DB
          switchToWorkspace(newWorkspace.id)

          // Update URL to include new workspace ID
          router.push(`/worker/${newWorkspace.id}`)
        }
        setIsWorkspacesLoading(false)
      })
      .catch((err) => {
  ...
  const handleDeleteWorkspace = async (id: string) => {
    ...
    try {
      ...
      if (activeWorkspace?.id === id && updatedWorkspaces.length > 0) {
        ...
        useWorkerRegistry.getState().handleWorkspaceDeletion(newWorkspaceId)
  ...
  const workspaceUrl = activeWorkspace ? `/worker/${activeWorkspace.id}` : '/w'
