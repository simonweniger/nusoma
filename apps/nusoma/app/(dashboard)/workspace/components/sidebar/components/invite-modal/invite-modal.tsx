...
import { PermissionType } from '@nusoma/database/schema'
...
const PermissionsTable = ({
  ...
  const currentUser: UserPermissions | null = session?.user?.email
    ? existingUsers.find((user) => user.isCurrentUser) || {
        ...
        permissionType: PermissionType.ADMIN, // Fallback if not found in workspace users
        isCurrentUser: true,
      }
    : null
...
export function InviteModal({ open, onOpenChange }: InviteModalProps) {
  ...
  const addEmail = (email: string) => {
    ...
    setUserPermissions((prev) => [
      ...prev,
      {
        ...
        permissionType: PermissionType.READ, // Default: read access
      },
    ])
  ...
  return (
    ...
    >
      <DialogContent className='flex flex-col gap-0 overflow-hidden p-0 sm:max-w-[600px]'>
