...
import type { Variable, VariableType } from '@nusoma/types/variables'
...
export function Variables({ panelWidth }: VariablesProps) {
  const { activeWorkerId, workers } = useWorkerRegistry()
  const {
  ...
  // Get variables for the current worker
  const workerVariables = activeWorkerId ? getVariablesByWorkerId(activeWorkerId) : []

  // Load variables when active worker changes
  useEffect(() => {
    if (activeWorkerId) {
      loadVariables(activeWorkerId)
    }
  }, [activeWorkerId, loadVariables])
  ...
  useEffect(() => {
    ...
    Object.keys(editorRefs.current).forEach((id) => {
      if (!workerVariables.some((v) => v.id === id)) {
