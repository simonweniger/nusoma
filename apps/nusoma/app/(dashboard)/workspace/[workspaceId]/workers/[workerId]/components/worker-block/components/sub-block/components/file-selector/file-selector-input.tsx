...
export function FileSelectorInput({
  ...
  const [_messageInfo, setMessageInfo] = useState<TeamsMessageInfo | null>(null)
  const [selectedCalendarId, setSelectedCalendarId] = useState<string>('')
  const [_calendarInfo, setCalendarInfo] = useState<GoogleCalendarInfo | null>(null)
  ...
  const _value = isPreview ? previewValue : getValue(blockId, subBlock.id)
    ...
    return (
      <TooltipProvider>
        <Tooltip>
          ...
          {(!botToken || !serverId) && (
            <TooltipContent side='top'>
              <p>{botToken ? 'Please select a Server first' : 'Please enter a Bot Token first'}</p>
            </TooltipContent>
          )}
        </Tooltip>
      </TooltipProvider>
  ...
  if (isMicrosoftTeams) {
    ...
    // Get the teamId from worker parameters for channel selector
