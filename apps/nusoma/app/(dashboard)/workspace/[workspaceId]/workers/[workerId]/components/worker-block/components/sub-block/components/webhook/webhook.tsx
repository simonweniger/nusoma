...
import {
  ...
  TelegramIcon,
  WhatsAppIcon,
} from '@nusoma/design-system/components/icons'
...
export const WEBHOOK_PROVIDERS: { [key: string]: WebhookProvider } = {
  ...
  generic: {
    ...
    icon: (props) => (
      ...
      >
        <span className='font-medium text-white text-xs'>nusoma</span>
      </div>
    ),
    configFields: {
      token: {
...
export function WebhookConfig({
  ...
  // Get worker store function to update webhook status
  const setWebhookStatus = useWorkerStore((state) => state.setWebhookStatus)
  ...
  useEffect(() => {
    ...
    const checkWebhook = async () => {
      setIsLoading(true)
      try {
        // Check if there's a webhook for this worker
  ...
  const handleDeleteWebhook = async () => {
    ...
    try {
      ...
      // Reset the startWorker field to manual
      useSubBlockStore.getState().setValue(blockId, 'startWorker', 'manual')

      // Remove webhook-specific fields from the block state
      const store = useSubBlockStore.getState()
      const workerValues = store.workerValues[workerId] || {}
      const blockValues = { ...workerValues[blockId] }
      ...
      store.setValue(blockId, 'startWorker', 'manual')
