...
import type { SerializedWorker } from '@/serializer/types'
...
const logger = createLogger('useWorkerExecution')
...
export function useWorkerExecution() {
  const { blocks, edges, loops, parallels } = useWorkerStore()
  ...
  const { getVariablesByWorkerId, variables } = useVariablesStore()
  const {
    isExecuting,
    isDebugging,
    pendingBlocks,
  ...
  const persistLogs = async (
    ...
    try {
      ...
      const response = await fetch(`/api/workers/${activeWorkerId}/log`, {
        method: 'POST',
        headers: {
  ...
  const handleRunWorker = useCallback(
    async (workerInput?: any) => {
      ...
      if (activeTab !== 'config' && activeTab !== 'test') {
        setActiveTab('test')
      }
      ...
      const isChatExecution =
        activeTab === 'test' &&
        workerInput &&
        typeof workerInput === 'object' &&
        'input' in workerInput
      ...
      if (isChatExecution) {
        const stream = new ReadableStream({
          async start(controller) {
            ...
            try {
              const result = await executeWorker(workerInput, onStream)
      ...
      const executionId = uuidv4()
      try {
        const result = await executeWorker(workerInput)
          ...
          if (activeWorkerId) {
            addNotification(
              ...
              result.success
                ...
                : `Worker execution failed: ${result.error || 'Unknown error'}`,
              activeWorkerId
            )
          }
          persistLogs(executionId, result).catch((err) => {
  ...
  const executeWorker = async (
    ...
    const workerVars = activeWorkerId ? getVariablesByWorkerId(activeWorkerId) : []
    const workerVariables = workerVars.reduce(
      (acc, variable) => {
    ...
    const worker = new Serializer().serializeWorker(mergedStates, edges, loops, parallels)
    ...
    const executorOptions: ExecutorOptions = {
      ...
      contextExtensions: {
        ...
        selectedOutputIds,
        edges: worker.connections.map((conn) => ({
          source: conn.source,
          target: conn.target,
        })),
        onStream,
    ...
    // Execute worker
    return newExecutor.execute(activeWorkerId || '')
  }
  ...
  const handleExecutionError = (error: any) => {
    ...
    let notificationMessage = 'Worker execution failed'
    ...
    try {
      addNotification('error', notificationMessage, activeWorkerId || '')
    } catch (notificationError) {
      logger.error('Error showing error notification:', notificationError)
      console.error('Worker execution failed:', errorMessage)
    }
  ...
  /**
   * Handles stepping through worker execution in debug mode
   */
  const handleStepDebug = useCallback(async () => {
    ...
    try {
      ...
      try {
        addNotification('error', `Debug step failed: ${errorMessage}`, activeWorkerId || '')
      } catch (notificationError) {
  ...
  const handleResumeDebug = useCallback(async () => {
    ...
    try {
      ...
      try {
        addNotification('info', 'Resuming worker execution until completion', activeWorkerId || '')
      } catch (notificationError) {
        logger.error('Error showing resume notification:', notificationError)
        console.info('Resuming worker execution until completion')
      }
      ...
      try {
        addNotification(
          ...
          currentResult.success
            ? 'Worker completed successfully'
            : `Worker execution failed: ${currentResult.error}`,
          activeWorkerId || ''
        )
      } catch (notificationError) {
        logger.error('Error showing completion notification:', notificationError)
        console.info('Worker execution completed')
      }
      ...
      // Safely show error notification
      try {
        addNotification('error', `Resume execution failed: ${errorMessage}`, activeWorkerId || '')
      } catch (notificationError) {
