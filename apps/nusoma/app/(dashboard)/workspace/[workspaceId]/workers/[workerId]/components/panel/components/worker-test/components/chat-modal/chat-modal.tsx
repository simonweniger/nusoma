...
import { JSONView } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/components/panel/components/console/components/json-view/json-view'
import { useWorkerExecution } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/hooks/use-worker-execution'
      ...
      <div className='px-4 py-5'>
        <div className='mx-auto max-w-3xl'>
          <div className='flex justify-end'>
            <div className='max-w-[80%] rounded-3xl bg-[#F4F4F4] px-4 py-3 shadow-elevation-low dark:bg-primary/10'>
...
export function ChatModal({ open, onOpenChange, chatMessage, setChatMessage }: ChatModalProps) {
  ...
  const handleSendMessage = async () => {
    if (!chatMessage.trim() || !activeWorkerId || isExecuting) {
      return
    }
    ...
    // Execute the worker to generate a response
    await handleRunWorker({ input: sentMessage })
  ...
  return (
    <div className='fixed inset-0 z-[100] flex flex-col bg-background'>
          ...
          {workerMessages.length === 0 ? (
            <div className='flex h-full flex-col items-center justify-center px-4 py-10'>
              <div className='space-y-2 text-center'>
                ...
                <p className='text-muted-foreground text-sm'>Ask me anything about your worker.</p>
              </div>
            </div>
          ) : (
            workerMessages.map((message) => <ModalChatMessage key={message.id} message={message} />)
          )}
      ...
      <div className='bg-background p-4'>
        <div className='mx-auto max-w-3xl'>
          <div className='relative rounded-2xl border bg-background shadow-elevation-low'>
            <Input
              ref={inputRef}
              value={chatMessage}
          ...
          <div className='mt-2 text-center text-muted-foreground text-xs'>
            <p>
              {activeWorkerId
                ? 'Your messages will be processed by the active worker'
