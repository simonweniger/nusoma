...
import { Code, FileJson, Trash2, Wand2 } from 'lucide-react'
...
import { useCodeGeneration } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/hooks/use-code-generation'
import {
  checkTagTrigger,
  TagDropdown,
...
export function CustomToolModal({
  ...
  const handleClose = () => {
    ...
    if (schemaGeneration.isStreaming) {
  ...
  // Pure validation function that doesn't update state
  const validateFunctionCode = (_code: string): boolean => {
  ...
  const handleJsonSchemaChange = (value: string) => {
    ...
    if (schemaGeneration.isLoading || schemaGeneration.isStreaming) {
      return
    }
    setJsonSchema(value)
    if (schemaError) {
  ...
  const handleFunctionCodeChange = (value: string) => {
    ...
    if (textarea) {
      ...
      const currentCol = lines.at(-1)?.length || 0
  ...
  const handleKeyDown = (e: React.KeyboardEvent<Element>) => {
    ...
    if ((showEnvVars || showTags) && ['ArrowDown', 'ArrowUp', 'Enter'].includes(e.key)) {
      e.preventDefault()
      e.stopPropagation()
    }
  }
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className='flex h-[80vh] flex-col gap-0 p-0 sm:max-w-[700px]'>
              ...
              {activeSection === 'schema' && (
                <CodePromptBar
                  ...
                  onSubmit={(prompt: string) =>
                    schemaGeneration.generateStream({
                      prompt,
                      context: jsonSchema,
                    })
                  }
                ...
                <div className='mb-1 flex min-h-6 items-center justify-between'>
                  <div className='flex items-center gap-2'>
                    ...
                    <Button
                      variant='ghost'
                      size='icon'
                      className='h-5 w-5 rounded-full border border-transparent bg-muted/80 p-0 text-muted-foreground shadow-elevation-low transition-all duration-200 hover:border-primary/20 hover:bg-muted hover:text-primary hover:shadow'
                      onClick={() => {
