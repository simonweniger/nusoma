...
import { Dialog } from '@nusoma/design-system/components/ui/dialog'
...
export function ScheduleConfig({
  ...
  // Get worker state from store
  const setScheduleStatus = useWorkerStore((state) => state.setScheduleStatus)

  // Get the schedule type from the block state
  const [scheduleType] = useSubBlockValue(blockId, 'scheduleType')

  // Get the startWorker value to determine if scheduling is enabled
  // and expose the setter so we can update it
  const [startWorker, setStartWorker] = useSubBlockValue(blockId, 'startWorker')
  //const isScheduleEnabled = startWorker === 'schedule'
  ...
  const checkSchedule = async () => {
    setIsLoading(true)
    try {
      // Check if there's a schedule for this worker, passing the mode parameter
  ...
  useEffect(() => {
    ...
    checkSchedule()
  }, [workerId, scheduleType, isModalOpen, refreshCounter])
  ...
  const handleSaveSchedule = async (): Promise<boolean> => {
    ...
    try {
      // 1. First, update the startWorker value in SubBlock store to 'schedule'
      setStartWorker('schedule')
      ...
      if (!activeWorkerId) {
        setError('No active worker found')
        return false
      }

      // Update the SubBlock store directly to ensure the value is set correctly
      const subBlockStore = useSubBlockStore.getState()
      subBlockStore.setValue(blockId, 'startWorker', 'schedule')

      // Give React time to process the state update
      await new Promise((resolve) => setTimeout(resolve, 200))

      // 3. Get the fully merged current state with updated values
      // This ensures we send the complete, correct worker state to the backend
      const currentWorkerWithValues = getWorkerWithValues(activeWorkerId)
      if (!currentWorkerWithValues) {
        setError('Failed to get current worker state')
        return false
      }
      ...
      const response = await fetch('/api/schedules', {
        ...
        body: JSON.stringify({
          workerId,
          state: currentWorkerWithValues.state,
        }),
      })
      ...
      // 6. Update the schedule status and trigger a worker update
      setScheduleStatus(true)

      // 7. Tell the worker store that the state has been saved
      const workerStore = useWorkerStore.getState()
      workerStore.updateLastSaved()
      workerStore.triggerUpdate()
  ...
  const handleDeleteSchedule = async (): Promise<boolean> => {
    ...
    setIsDeleting(true)
    try {
      // 1. First update the worker state to disable scheduling
      setStartWorker('manual')
      ...
      const subBlockStore = useSubBlockStore.getState()
      subBlockStore.setValue(blockId, 'startWorker', 'manual')

      // 3. Update the worker store
      const workerStore = useWorkerStore.getState()
      workerStore.triggerUpdate()
      workerStore.updateLastSaved()
