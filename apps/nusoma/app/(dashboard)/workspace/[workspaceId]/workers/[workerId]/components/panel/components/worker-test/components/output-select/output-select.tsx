...
export function OutputSelect({
  ...
  // Get worker outputs for the dropdown
  ...
  const selectedOutputsDisplayText = useMemo(() => {
    ...
    // Ensure all selected outputs exist in the workerOutputs array
    const validOutputs = selectedOutputs.filter((id) => workerOutputs.some((o) => o.id === id))
    ...
    if (validOutputs.length === 1) {
      const output = workerOutputs.find((o) => o.id === validOutputs[0])
      if (output) {
  ...
  }, [selectedOutputs, workerOutputs, placeholder])
  ...
  }, [selectedOutputs, workerOutputs])

  // Group output options by block
  const groupedOutputs = useMemo(() => {
    const groups: Record<string, typeof workerOutputs> = {}
    const blockDistances: Record<string, number> = {}
    const edges = useWorkerStore.getState().edges
    ...
    return groupsArray.reduce(
      ...
      {} as Record<string, typeof workerOutputs>
    )
  }, [workerOutputs, blocks])

  // Get block color for an output
  const getOutputColor = (_blockId: string, blockType: string) => {
  ...
  return (
    <div className='relative' ref={dropdownRef}>
      <button
        ...
        disabled={workerOutputs.length === 0 || disabled}
      >
        {selectedOutputInfo ? (
      ...
      {isOutputDropdownOpen && workerOutputs.length > 0 && (
