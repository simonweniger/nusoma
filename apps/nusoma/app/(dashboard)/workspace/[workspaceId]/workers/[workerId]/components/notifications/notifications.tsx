...
import { ErrorIcon } from '@nusoma/design-system/components/icons'
...
import { CopyButton } from '@nusoma/design-system/components/ui/copy-button'
...
export function NotificationAlert({ notification, isFading, onHide }: NotificationAlertProps) {
  const { id, type, message, options, workerId } = notification
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [showApiKey, setShowApiKey] = useState(false)
  const { setDeploymentStatus } = useWorkerRegistry()

  // Get deployment status from registry using notification's workerId, not activeWorkerId
  const deploymentStatus = useWorkerRegistry((state) =>
    state.getWorkerDeploymentStatus(workerId || null)
  )
  ...
  const updateDeploymentStatus = (isDeployed: boolean, deployedAt?: Date) => {
    // Update deployment status in worker store
    setDeploymentStatus(workerId || null, isDeployed, deployedAt)

    // Manually update the needsRedeployment flag in worker store
    useWorkerStore.getState().setNeedsRedeploymentFlag(false)
  }
          ...
          <div className='relative flex items-start py-1'>
            ...
            <div className='mx-4 flex-1 space-y-2 pt-[3.5px] pr-4'>
              ...
              <AlertDescription className='space-y-4'>
                <p>{!isDeployed ? 'Worker currently not deployed' : message}</p>
                    ...
                    {options?.needsRedeployment && (
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            // Call the deploy endpoint to redeploy the worker
                            const response = await fetch(`/api/workers/${workerId}/deploy`, {
                              method: 'POST',
                            })

                            if (!response.ok) throw new Error('Failed to redeploy worker')
