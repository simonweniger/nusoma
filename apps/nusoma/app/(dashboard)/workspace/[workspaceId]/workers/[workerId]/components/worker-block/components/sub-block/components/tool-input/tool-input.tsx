...
import { Toggle } from '@nusoma/design-system/components/ui/toggle'
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
export function ToolInput({
  ...
  const isWide = useWorkerStore((state) => state.blocks[blockId]?.isWide)
  // Get the raw tools data instead of calling getAllTools() which creates new arrays
  const customToolsData = useCustomToolsStore((state) => state.tools)
  const customTools = useMemo(() => Object.values(customToolsData), [customToolsData])
  const subBlockStore = useSubBlockStore()
  const isAutoFillEnvVarsEnabled = useGeneralStore((state) => state.isAutoFillEnvVarsEnabled)

  // Get the current model from the 'model' subblock - memoize to prevent infinite calls
  const modelValue = useMemo(() => {
    return subBlockStore.getValue(blockId, 'model')
  }, [subBlockStore, blockId])
  ...
  // Memoize toolBlocks to prevent creating new arrays on every render
  const toolBlocks = useMemo(() => {
    return getAllBlocks().filter((block) => block.category === 'tools')
  }, [])
  ...
  const handleEditCustomTool = (toolIndex: number) => {
    ...
    // Find the tool ID from the custom tools list based on the function name
    const _existingTool = customTools.find(
  ...
  const handleDeleteTool = (toolId: string) => {
    // Find any instances of this tool in the current worker and remove them
    ...
    // Update the worker value if any tools were removed
  ...
  const handleParamChange = (toolIndex: number, paramId: string, paramValue: string) => {
    ...
    // Update the value in the worker
    setStoreValue(
      selectedTools.map((tool, index) =>
        index === toolIndex
          ? {
