...
import { Calendar as CalendarComponent } from '@nusoma/design-system/components/ui/calendar'
import {
  DialogContent,
  DialogFooter,
  DialogHeader,
...
export function ScheduleModal({
  ...
  // Get the startWorker value at the component level
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Make sure the block's startWorker field is set to 'schedule'
      logger.debug('Current startWorker value:', startWorker)

      // Important: Set startWorker to 'schedule' in two ways for maximum reliability
      // 1. Via the hook which will trigger a state update
      if (startWorker !== 'schedule') {
        logger.debug('Setting startWorker to schedule via hook')
      ...
      logger.debug('Setting startWorker to schedule directly in store')
      useSubBlockStore.getState().setValue(blockId, 'startWorker', 'schedule')
            ...
            {scheduleType === 'hourly' && (
              <div className='space-y-1'>
                ...
                <p className='text-muted-foreground text-xs'>
                  Specify which minute of each hour the worker should run (0-59)
                </p>
              </div>
            )}
            ...
            {scheduleType === 'monthly' && (
              <div className='space-y-4'>
                <div className='space-y-1'>
                  ...
                  />
                  <p className='text-muted-foreground text-xs'>
                    Specify which day of the month the worker should run (1-31)
                  </p>
                </div>
