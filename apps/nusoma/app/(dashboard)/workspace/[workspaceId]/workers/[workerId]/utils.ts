 ...
 * @param getNodes Function to retrieve all nodes from ReactFlow
 * @param blocks Block states from worker store
...
/**
 * Detects the predominant handle orientation in the worker
 * @param blocks Block states from worker store
...
/**
 * Analyzes the worker graph using topological sort to determine execution layers
 * and properly handle parallel execution paths, disabled blocks, and terminal blocks
 * @param blocks Block states from worker store
 * @param edges Edge connections from worker store
 * @returns Map of block IDs to their layer numbers and additional graph data
 */
export const analyzeWorkerGraph = (
  blocks: Record<string, any>,
  edges: any[]
): {
...
/**
 * Calculates auto-layout positions for worker blocks with improved spacing and alignment
...
export const calculateAutoLayout = (
  ...
  if (alignByLayer) {
    const { parallelGroups, maxLayer, disabledBlocks, terminalBlocks, orphanedBlocks } =
      analyzeWorkerGraph(topLevelBlocks, edges)
