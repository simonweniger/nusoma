...
import { MicrosoftTeamsIcon } from '@nusoma/design-system/components/icons'
...
import { saveToStorage } from '@/stores/workers/persistence'
...
export function TeamsMessageSelector({
  ...
  }, [selectedCredentialId, selectedTeamId, onMessageInfoChange, workerId])

  // Fetch channels for a selected team
  const fetchChannels = useCallback(
    async (teamId: string) => {
      if (!selectedCredentialId || !teamId) {
        return
      }
    ...
    [selectedCredentialId, selectedChannelId, onMessageInfoChange, workerId]
  )
  ...
  const fetchChats = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/tools/microsoft-teams/chats', {
        ...
        body: JSON.stringify({
          credential: selectedCredentialId,
          workerId: workerId, // Pass the workerId for server-side authentication
        }),
      })
  ...
  }, [selectedCredentialId, selectedChatId, onMessageInfoChange, workerId])
  ...
  const restoreTeamSelection = useCallback(
    async (teamId: string) => {
      if (!selectedCredentialId || !teamId || selectionType !== 'team') {
        return
      }
    ...
    [selectedCredentialId, selectionType, onMessageInfoChange, workerId]
  )

  // Restore chat selection on page refresh
  const restoreChatSelection = useCallback(
    async (chatId: string) => {
      if (!selectedCredentialId || !chatId || selectionType !== 'chat') {
        return
      }
  ...
  const restoreChannelSelection = useCallback(
    async (channelId: string) => {
      if (!selectedCredentialId || !channelId || selectionType !== 'channel') {
        return
      }
