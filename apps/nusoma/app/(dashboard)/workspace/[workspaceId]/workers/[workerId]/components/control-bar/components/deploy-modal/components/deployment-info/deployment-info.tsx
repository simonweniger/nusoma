...
import { ApiEndpoint } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/components/control-bar/components/deploy-modal/components/deployment-info/components/api-endpoint/api-endpoint'
import { ApiKey } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/components/control-bar/components/deploy-modal/components/deployment-info/components/api-key/api-key'
import { DeployStatus } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/components/control-bar/components/deploy-modal/components/deployment-info/components/deploy-status/deploy-status'
import { ExampleCommand } from '@/app/(dashboard)/workspace/[workspaceId]/workers/[workerId]/components/control-bar/components/deploy-modal/components/deployment-info/components/example-command/example-command'
import { useNotificationStore } from '@/stores/notifications/store'
import type { WorkerState } from '@/stores/workers/worker/types'
import { DeployedWorkerModal } from '../../../deployment-controls/components/deployed-worker-modal'
...
export function DeploymentInfo({
  ...
  const handleViewDeployed = async () => {
    if (!workerId) {
      addNotification('error', 'Cannot view deployment: Worker ID is missing', null)
      return
    }
    ...
    if (!isLoadingDeployedState) {
      addNotification('error', 'Cannot view deployment: No deployed state available', workerId)
    }
  }
    ...
    <>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Are you sure you want to undeploy this worker? This will remove the API endpoint
      ...
      {deployedState && (
        <DeployedWorkerModal
          ...
          deployedWorkerState={deployedState}
        />
      )}
    </>
  )
