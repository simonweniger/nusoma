 ...
 * @param onRunWorker - Function to run when Cmd/Ctrl+Enter is pressed
 * @param isDisabled - Whether shortcuts should be disabled
 */
export function useKeyboardShortcuts(onRunWorker: () => void, isDisabled = false) {
  // Memoize the platform detection
  const isMac = useMemo(() => isMacPlatform(), [])

  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // Run worker with Cmd+Enter (Mac) or Ctrl+Enter (Windows/Linux)
