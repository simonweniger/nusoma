...
import { env } from './env'
import { type ErrorCode, NusomaError } from './errors'
...
export const fetcher = async (url: string) => {
  ...
  if (!response.ok) {
    ...
    throw new NusomaError(code as ErrorCode, cause)
  }
...
export async function fetchWithErrorHandlers(input: RequestInfo | URL, init?: RequestInit) {
  try {
    ...
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      throw new NusomaError('offline:chat')
    }
...
/**
 * Ensures timeout value is always non-negative to prevent Node.js TimeoutNegativeWarning
 * @param timeout - The timeout value in milliseconds
 * @param defaultTimeout - Default timeout to use if input is negative (default: 1000ms)
 * @returns A non-negative timeout value
 */
export function ensurePositiveTimeout(timeout: number, defaultTimeout = 1000): number {
  if (timeout < 0) {
    console.warn(
      `Negative timeout detected (${timeout}ms), using default timeout (${defaultTimeout}ms) instead`
    )
...
/**
 * Generates a standardized API key with the 'nus_' prefix
 * @returns A new API key string
 */
export function generateApiKey(): string {
  return `nus_${nanoid(32)}`
}
...
export function getRotatingApiKey(provider: string): string {
  ...
  if (provider === 'openai') {
    ...
    if (env.OPENAI_API_KEY_1) {
      keys.push(env.OPENAI_API_KEY_1)
    }
    if (env.OPENAI_API_KEY_2) {
      keys.push(env.OPENAI_API_KEY_2)
    }
    if (env.OPENAI_API_KEY_3) {
      keys.push(env.OPENAI_API_KEY_3)
    }
  } else if (provider === 'anthropic') {
    if (env.ANTHROPIC_API_KEY) {
      keys.push(env.ANTHROPIC_API_KEY)
    }
    if (env.ANTHROPIC_API_KEY_1) {
      keys.push(env.ANTHROPIC_API_KEY_1)
    }
    if (env.ANTHROPIC_API_KEY_2) {
      keys.push(env.ANTHROPIC_API_KEY_2)
    }
    if (env.ANTHROPIC_API_KEY_3) {
      keys.push(env.ANTHROPIC_API_KEY_3)
    }
  }

  if (keys.length === 0) {
    throw new Error(
      `No API keys configured for rotation. Please configure ${provider.toUpperCase()}_API_KEY or ${provider.toUpperCase()}_API_KEY_1, ${provider.toUpperCase()}_API_KEY_2, or ${provider.toUpperCase()}_API_KEY_3.`
    )
  }
