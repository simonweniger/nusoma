...
describe('generateApiKey', () => {
  it.concurrent('should generate API key with nus_ prefix', () => {
    const key = generateApiKey()
    expect(key).toMatch(/^nus_/)
  })
  ...
  it.concurrent('should generate API keys of correct length', () => {
    const key = generateApiKey()
    // Expected format: 'nus_' + 32 random characters
...
describe('validateName', () => {
  ...
  it.concurrent('should handle mixed valid and invalid characters', () => {
    const result = validateName('my-worker@2023!')
    expect(result).toBe('myworker2023')
  })
