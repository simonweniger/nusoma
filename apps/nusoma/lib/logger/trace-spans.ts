import type { TraceSpan } from '@/lib/logger/types'
...
export function buildTraceSpans(result: ExecutionResult): {
  ...
  // Create a map to track parent-child relationships from worker structure
  // This helps distinguish between actual parent-child relationships vs parallel execution
  const parentChildMap = new Map<string, string>()

  // If we have worker information in the logs, extract parent-child relationships
  // Define connection type inline for now
  type Connection = { source: string; target: string }
  const workerConnections: Connection[] = result.metadata?.workerConnections || []
  if (workerConnections.length > 0) {
    // Build the connection map from worker connections
    workerConnections.forEach((conn: Connection) => {
  ...
  sortedLogs.forEach((log) => {
    ...
    // Check if this block has a direct parent in the worker
  ...
  // This happens when we don't have worker connection information
  if (rootSpans.length === 0 && workerConnections.length === 0) {
    ...
    sortedLogs.forEach((log) => {
      ...
      if (log.blockType === 'agent' || log.blockType === 'worker') {
        spanStack.push(span)
      }
    })
  }
  ...
  // Create a synthetic worker span that represents the entire execution
  ...
  if (rootSpans.length > 0 && result.metadata) {
    ...
    // Calculate actual worker duration from earliest start to latest end
    // This correctly accounts for parallel execution
    const actualWorkerDuration = latestEnd - earliestStart

    // Check if any spans have errors to determine overall worker status
    ...
    // Create the worker span
    const workerSpan: TraceSpan = {
      id: 'worker-execution',
      name: 'Worker Execution',
      type: 'worker',
      duration: actualWorkerDuration, // Always use actual duration for the span
    ...
    return { traceSpans: [workerSpan], totalDuration: actualWorkerDuration }
  }
