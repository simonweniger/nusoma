...
import { userStats, worker, workerLogs } from '@nusoma/database/schema'
...
import { stripCustomToolPrefix } from '../workers/utils'
...
export async function persistLog(log: LogEntry) {
  await db.insert(workerLogs).values(log)
}

/**
 * Persists logs for a worker execution, including individual block logs and the final result
...
export async function persistExecutionLogs(
  ...
  try {
    // Get the worker record to get the userId
    ...
    const userId = workerRecord.userId
    ...
    for (const log of result.logs || []) {
      ...
      if (
        ...
        if (log.output.response) {
          ...
          if (response.cost) {
            ...
            // Accumulate costs for worker-level summary
      ...
      await persistLog({
        ...
        message: log.success
          ? `Block ${log.blockName || log.blockId} (${log.blockType || 'unknown'}): ${log.output?.response?.content ||
    ...
    // Create worker-level metadata with aggregated cost information
    const workerMetadata: any = {
      traceSpans: (result as any).traceSpans || [],
      totalDuration: (result as any).totalDuration || actualDuration,
    }

    // Add accumulated cost data to worker-level log
    if (totalCost > 0) {
      workerMetadata.cost = {
        model: primaryModel,
      ...
      if (primaryModel && result.logs && result.logs.length > 0) {
        ...
        for (const log of result.logs) {
          if (log.output?.response?.cost?.pricing) {
            workerMetadata.cost.pricing = log.output.response.cost.pricing
            break
          }
        }
      }
      ...
      if (
        ...
        (!workerMetadata.cost || workerMetadata.cost.total <= 0)
      ) {
...
/**
 * Persists an error log for a worker execution
 * @param workerId - The ID of the worker
 * @param executionId - The ID of the execution
...
function getTriggerSuccessMessage(
  ...
  switch (triggerType) {
    case 'api':
      return 'API worker executed successfully'
    case 'webhook':
      return 'Webhook worker executed successfully'
    case 'schedule':
      return 'Scheduled worker executed successfully'
    case 'manual':
      return 'Manual worker executed successfully'
    case 'chat':
      return 'Chat worker executed successfully'
    default:
