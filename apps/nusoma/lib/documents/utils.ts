...
import { ensurePositiveTimeout } from '@/lib/utils'
...
export async function retryWithExponentialBackoff<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Ensure the delay is always positive to prevent TimeoutNegativeWarning
      const safeDelay = ensurePositiveTimeout(actualDelay, 1000)

      logger.info(
        `Retrying in ${Math.round(safeDelay)}ms (attempt ${attempt + 1}/${maxRetries + 1})`
      )
