...
import { PermissionType, permissions, user } from '@nusoma/database/schema'
 ...
 * @param entityType - The type of entity (e.g., 'workspace', 'worker', etc.)
  ...
  const result = await db
    ...
    .where(
      and(
        ...
        eq(permissions.permissionType, PermissionType.ADMIN)
      )
    )
    .limit(1)
