...
vi.mock('@nusoma/database/schema', () => ({
  permissions: {
...
import { permissions, user } from '@nusoma/database/schema'
...
describe('Permission Utils', () => {
  ...
  describe('getUserEntityPermissions', () => {
    ...
    it.concurrent('should return single permission when user has only one', async () => {
      ...
      const result = await getUserEntityPermissions('user123', 'worker', 'worker789')
    ...
    it.concurrent('should correctly prioritize write over read', async () => {
      ...
      const result = await getUserEntityPermissions('write-user', 'worker', 'worker-write')
