...
import { environment, userStats, webhook } from '@nusoma/database/schema'
...
import { mergeSubblockStateAsync } from '@/stores/workers/server-utils'
...
export function formatWebhookInput(
  ...
  if (foundWebhook.provider === 'telegram') {
    ...
    if (message) {
      ...
      return {
        input, // Primary worker input - the message content
        telegram: {
          message: {
...
/**
 * Execute worker with the provided input
 */
export async function executeWorkerFromPayload(
  foundWorker: any,
  input: any,
  executionId: string,
  requestId: string
  ...
  logger.info(`[${requestId}] Preparing to execute worker`, {
    workerId: foundWorker.id,
    executionId,
  ...
  if (!input) {
    logger.warn(`[${requestId}] Empty input for worker execution, creating empty object`)
    input = {}
  }
  ...
  logger.debug(`[${requestId}] Worker input format:`, {
  ...
  try {
    ...
    logger.debug(`[${requestId}] Loading worker ${foundWorker.id} from normalized tables`)
    const normalizedData = await loadWorkerFromNormalizedTables(foundWorker.id)

    if (!normalizedData) {
      logger.error(`[${requestId}] TRACE: No normalized data found for worker`, {
        workerId: foundWorker.id,
        hasNormalizedData: false,
      })
      throw new Error(`Worker ${foundWorker.id} data not found in normalized tables`)
    }

    // Use normalized data for execution
    const { blocks, edges, loops, parallels } = normalizedData
    logger.info(`[${requestId}] Loaded worker ${foundWorker.id} from normalized tables`)

    // DEBUG: Log state information
    logger.debug(`[${requestId}] TRACE: Retrieved worker state from normalized tables`, {
      workerId: foundWorker.id,
    ...
    logger.debug(
      `[${requestId}] Merging subblock states for worker ${foundWorker.id} (Execution: ${executionId})`
    )

    const mergeStartTime = Date.now()
    const mergedStates = await mergeSubblockStateAsync(blocks, foundWorker.id)
    ...
    const [userEnv] = await db
      .select()
      .from(environment)
      .where(eq(environment.userId, foundWorker.userId))
      .limit(1)
    ...
    // Serialize and get worker variables
    ...
    let workerVariables = {}
    if (foundWorker.variables) {
      try {
        if (typeof foundWorker.variables === 'string') {
          workerVariables = JSON.parse(foundWorker.variables)
        } else {
          workerVariables = foundWorker.variables
        }
      } catch (error) {
        logger.error(
          `[${requestId}] Failed to parse worker variables: ${foundWorker.id} (Execution: ${executionId})`,
          error
        )
      }
    }
    ...
    logger.debug(`[${requestId}] TRACE: Worker serialized`, {
      duration: `${Date.now() - serializeStartTime}ms`,
      hasWorkerVars: Object.keys(workerVariables).length > 0,
    })

    logger.debug(`[${requestId}] Starting worker execution`, {
      executionId,
      blockCount: Object.keys(processedBlockStates).length,
    })
    ...
    if (Object.keys(processedBlockStates).length === 0) {
      logger.error(`[${requestId}] No blocks found in worker state - this will likely fail`)
    } else {
    ...
    // Ensure worker variables exist
    if (!workerVariables || Object.keys(workerVariables).length === 0) {
      logger.debug(`[${requestId}] No worker variables defined, using empty object`)
      workerVariables = {}
    }
    ...
    logger.info(`[${requestId}] TRACE: Creating worker executor`, {
      workerId: foundWorker.id,
      hasSerializedWorker: !!serializedWorker,
      blockCount: Object.keys(processedBlockStates).length,
      timestamp: new Date().toISOString(),
    })
    ...
    // Log worker execution start time for tracking
    const executionStartTime = Date.now()
    logger.info(`[${requestId}] TRACE: Executor instantiated, starting worker execution now`, {
      workerId: foundWorker.id,
      timestamp: new Date().toISOString(),
    })
    ...
    // This is THE critical line where the worker actually executes
    const result = await executor.execute(foundWorker.id)
    ...
    logger.info(`[${requestId}] TRACE: Worker execution completed`, {
      workerId: foundWorker.id,
      success: executionResult.success,
    ...
    logger.info(`[${requestId}] Worker execution finished`, {
      executionId,
      success: executionResult.success,
    ...
    if (executionResult.success) {
      await updateWorkerRunCounts(foundWorker.id)
      await db
        .update(userStats)
        .set({
        ...
        .where(eq(userStats.userId, foundWorker.userId))

      // DEBUG: Log stats update
      logger.debug(`[${requestId}] TRACE: Worker stats updated`, {
        workerId: foundWorker.id,
        userId: foundWorker.userId,
      })
    }
    ...
    await persistExecutionLogs(foundWorker.id, executionId, enrichedResult, 'webhook')
    ...
    logger.error(`[${requestId}] TRACE: Error during worker execution`, {
      workerId: foundWorker.id,
      executionId,
    ...
    logger.error(`[${requestId}] Error executing worker`, {
      workerId: foundWorker.id,
      executionId,
      error: error.message,
      stack: error.stack,
    ...
    await persistExecutionError(foundWorker.id, executionId, error, 'webhook')
...
export async function fetchAndProcessAirtablePayloads(
  ...
  try {
    ...
    try {
      accessToken = await getOAuthToken(workerData.userId, 'airtable')
      if (!accessToken) {
        logger.error(
      ...
      logger.error(
        `[${requestId}] Failed to get Airtable OAuth token for user ${workerData.userId}`,
        {
    ...
    // --- Execute Worker if we have changes (simplified - no lock check) ---
    if (finalConsolidatedChanges.length > 0) {
      try {
        ...
        logger.info(
          `[${requestId}] CRITICAL_TRACE: Beginning worker execution with ${finalConsolidatedChanges.length} Airtable changes`,
          {
            workerId: workerData.id,
        ...
        await executeWorkerFromPayload(workerData, input, requestId, requestId)

        // COMPLETION LOG - This will only appear if execution succeeds
        logger.info(`[${requestId}] CRITICAL_TRACE: Worker execution completed successfully`, {
          workerId: workerData.id,
          timestamp: new Date().toISOString(),
        })
      } catch (executionError: any) {
        // Errors logged within executeWorkerFromPayload
        logger.error(`[${requestId}] CRITICAL_TRACE: Worker execution failed with error`, {
          workerId: workerData.id,
        ...
        logger.error(
          `[${requestId}] Error during worker execution triggered by Airtable polling`,
          executionError
        )
      }
    } else {
...
export async function processWebhook(
  ...
  try {
    ...
    if (foundWebhook.provider === 'airtable') {
      ...
      await fetchAndProcessAirtablePayloads(foundWebhook, foundWorker, requestId)
    ...
    const input = formatWebhookInput(foundWebhook, foundWorker, body, request)
    ...
    // --- Execute Worker ---
    logger.info(
      `[${requestId}] Executing worker ${foundWorker.id} for webhook ${foundWebhook.id} (Execution: ${executionId})`
    )
    // Call the refactored execution function
    await executeWorkerFromPayload(foundWorker, input, executionId, requestId)

    // Since executeWorkerFromPayload handles logging and errors internally,
    ...
    // Catch errors *before* calling executeWorkerFromPayload (e.g., auth errors)
    logger.error(
