...
import { userStats, worker as workerTable } from '@nusoma/database/schema'
...
export async function getWorkerById(id: string) {
  const workers = await db.select().from(workerTable).where(eq(workerTable.id, id)).limit(1)
  return workers[0]
}

export async function updateWorkerRunCounts(workerId: string, runs = 1) {
  try {
    ...
    if (origin) {
      ...
      const response = await fetch(`${origin}/api/workers/${workerId}/stats?runs=${runs}`, {
        method: 'POST',
      })

      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.error || 'Failed to update worker stats')
      }

      return response.json()
    }
    logger.warn('No origin available, updating worker stats directly via DB')

    // Update worker directly through database
    await db
      .update(workerTable)
      .set({
        runCount: worker.runCount + runs,
        lastRunAt: new Date(),
      })
    ...
    logger.error('Error updating worker run counts:', error)
    throw error
  }
}
...
/**
 * Compare the current worker state with the deployed state to detect meaningful changes
 * @param currentState - The current worker state
 * @param deployedState - The deployed worker state
 * @returns True if there are meaningful changes, false if only position changes or no changes
 */
export function hasWorkerChanged(
  currentState: WorkerState,
  deployedState: WorkerState | null
): boolean {
  // If no deployed state exists, then the worker has changed
...
export const workerHasResponseBlock = (executionResult: ExecutionResult): boolean => {
  if (
