...
const resend = validResendAPIKEY
  ...
  : {
    emails: {
      send: async (...args: any[]) => {
        await logger.info('Email would have been sent in production. Details:', args)
      ...
      create: {
        before: async (session) => {
          try {
            ...
            await logger.info('No organizations found for user', {
              userId: session.userId,
            })

            return { data: session }
          } catch (error) {
            await logger.error('Error setting active organization', {
              error,
              userId: session.userId,
            })
            return { data: session }
    ...
    emailOTP({
      sendVerificationOTP: async (data: {
        ...
        try {
          ...
          const result = await resend.emails.send({
            from: 'nusoma <onboarding@nusoma.app>',
            to: data.email,
      ...
      config: [
        {
          ...
          scopes: ['user:email', 'repo', 'read:user', 'worker'],
    ...
    ...(stripeEnabled && stripeClient
      ? [
        stripe({
          ...
          onCustomerCreate: async ({ customer, stripeCustomer, user }, _request) => {
          ...
          subscription: {
            ...
            plans: [
              {
                ...
                limits: {
                  cost: env.PERSONAL_TIER_COST_LIMIT ?? 5,
                  sharingEnabled: 0,
              ...
              },
              {
                ...
                limits: {
                  cost: env.TEAM_TIER_COST_LIMIT ?? 40, // $40 per seat                  sharingEnabled: 1,
            ...
            getCheckoutSessionParams: ({ user, plan, subscription }, _request) => {
              if (plan.name === 'team') {
                return {
                  params: {
            ...
            onSubscriptionComplete: async ({
              ...
              await logger.info('Subscription created', {
            ...
            onSubscriptionDeleted: async ({
              ...
              await logger.info('Subscription deleted', {
                subscriptionId: subscription.id,
                referenceId: subscription.referenceId,
              })
            },
          ...
          sendInvitationEmail: async (data) => {
            try {
              ...
              await resend.emails.send({
                from: 'nusoma <team@nusoma.app>',
                to: invitation.email,
                subject: `${inviterName} has invited you to join ${organization.name} on nusoma`,
                html,
              })
            } catch (error) {
