...
export async function executeTool(
  ...
  try {
    ...
    // If it's a custom tool, use the async version with workerId
    if (toolId.startsWith('custom_')) {
      const workerId = params._context?.workerId
      tool = await getToolAsync(toolId, workerId)
    } else {
    ...
    if (contextParams.credential) {
      ...
      try {
        ...
        // Add workerId if it exists in params or context (only server-side)
        if (isServerSide) {
          const workerId = contextParams.workerId || contextParams._context?.workerId
          if (workerId) {
            tokenPayload.workerId = workerId
            logger.info(`[executeTool] Added workerId ${workerId} to token payload for ${toolId}`)
          }
        }
        ...
        if (contextParams.workerId) {
          contextParams.workerId = undefined
        }
      } catch (error) {
...
function validateClientSideParams(
  ...
  const internalParamSet = new Set(['_context', 'workerId', 'envVars'])
