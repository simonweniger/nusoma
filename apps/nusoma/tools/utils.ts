 ...
 * @param workerId Optional worker ID for server-side
 * @param getStore Optional function to get the store (useful for testing)
 */
export function createCustomToolRequestBody(
  ...
  return (params: Record<string, any>) => {
    ...
    return {
      ...
      workerId: workerId, // Pass workerId for server-side context
...
export async function getToolAsync(
  ...
  if (toolId.startsWith('custom_')) {
    return getCustomTool(toolId, workerId)
  }
...
function createToolConfig(customTool: any, customToolId: string): ToolConfig {
  ...
  return {
    ...
    transformResponse: async (response: Response, _params: Record<string, any>) => {
...
async function getCustomTool(
  ...
  try {
    ...
    // Add workerId as a query parameter if available
    if (workerId) {
    ...
    return {
      ...
      request: {
        ...
        method: 'POST',
        headers: () => ({ 'Content-Type': 'application/json' }),
        body: createCustomToolRequestBody(customTool, false, workerId),
        isInternalRoute: true,
      },
