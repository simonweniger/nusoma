...
export class LoopManager {
    ...
    for (const [loopId, loop] of Object.entries(this.loops)) {
      ...
      if (allBlocksInLoopExecuted) {
        ...
        if (currentIteration >= maxIterations) {
          ...
          // Activate the loop-end-source connections to continue worker after loop
  ...
  private allBlocksExecuted(nodeIds: string[], context: ExecutionContext): boolean {
    ...
    while (toVisit.length > 0) {
      ...
      // Get the block
      const block = context.worker?.blocks.find((b) => b.id === currentBlockId)
      if (!block) continue
   ...
   * @param blocks - All blocks in the worker
