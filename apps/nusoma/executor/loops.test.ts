...
import type { SerializedLoop, SerializedWorker } from '@/serializer/types'
...
describe('LoopManager', () => {
  ...
  const createWorkerWithLoop = (loop: SerializedLoop): SerializedWorker => ({
    version: '2.0',
    blocks: [
      {
        id: 'starter',
  ...
  beforeEach(() => {
    ...
    mockContext = {
      ...
      worker: createWorkerWithLoop(createBasicLoop()),
    }
  })
  ...
  describe('processLoopIterations', () => {
    ...
    test('should handle forEach loops with array items', async () => {
      ...
      mockContext.worker!.loops['loop-1'] = forEachLoop
  ...
  describe('allBlocksExecuted (private method testing through processLoopIterations)', () => {
    test('should handle router blocks with selected paths', async () => {
      // Create a worker with a router block inside the loop
      const worker = createWorkerWithLoop(createBasicLoop())
      worker.blocks[2].metadata!.id = 'router' // Make block-1 a router
      worker.connections = [
        { source: 'starter', target: 'loop-1' },
        { source: 'loop-1', target: 'block-1', sourceHandle: 'loop-start-source' },
    ...
    test('should handle condition blocks with selected paths', async () => {
      // Create a worker with a condition block inside the loop
      const worker = createWorkerWithLoop(createBasicLoop())
      worker.blocks[2].metadata!.id = 'condition' // Make block-1 a condition
      worker.connections = [
        { source: 'starter', target: 'loop-1' },
        { source: 'loop-1', target: 'block-1', sourceHandle: 'loop-start-source' },
    ...
    test('should handle error connections properly', async () => {
      // Create a worker with error handling inside the loop
      const worker = createWorkerWithLoop(createBasicLoop())
      worker.connections = [
        { source: 'starter', target: 'loop-1' },
        { source: 'loop-1', target: 'block-1', sourceHandle: 'loop-start-source' },
    ...
    test('should handle blocks with errors following error paths', async () => {
      ...
      worker.loops['loop-1'].nodes.push('error-handler')
      worker.connections = [
        { source: 'starter', target: 'loop-1' },
        { source: 'loop-1', target: 'block-1', sourceHandle: 'loop-start-source' },
        { source: 'block-1', target: 'block-2', sourceHandle: 'source' },
  ...
  describe('edge cases and error handling', () => {
    test('should handle empty loop nodes array', async () => {
      ...
      mockContext.worker!.loops['loop-1'] = emptyLoop
    ...
    test('should handle missing worker in context', async () => {
      mockContext.worker = undefined
    ...
    test('should handle missing loop configuration', async () => {
      // Remove loop from worker
      if (mockContext.worker) {
        mockContext.worker.loops = {}
      }
  ...
  describe('integration scenarios', () => {
    test('should handle multiple loops in worker', async () => {
      const loops = {
