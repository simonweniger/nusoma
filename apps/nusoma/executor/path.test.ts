...
describe('PathTracker', () => {
  ...
  let mockWorker: SerializedWorker
  ...
  beforeEach(() => {
    ...
    pathTracker = new PathTracker(mockWorker)
  })
  ...
  describe('updateExecutionPaths', () => {
    describe('router blocks', () => {
      it('should update router decision and activate selected path', () => {
        const blockState: BlockState = {
          output: { response: { selectedPath: { blockId: 'target-block' } } },
          executed: true,
          executionTime: 0,
        }
    ...
    describe('condition blocks', () => {
      it('should update condition decision and activate selected connection', () => {
        const blockState: BlockState = {
          output: { response: { selectedConditionId: 'condition-1' } },
          executed: true,
          executionTime: 0,
        }
    ...
    describe('regular blocks', () => {
      it('should activate outgoing connections on success', () => {
        const blockState: BlockState = {
          output: { response: { data: 'test data' } },
          executed: true,
          executionTime: 0,
        }
      ...
      it('should activate error connections on error', () => {
        ...
        const blockState: BlockState = {
          output: { error: 'test error', response: { error: 'test error' } },
          executed: true,
          executionTime: 0,
        }
        mockContext.blockStates.set('block1', blockState)
      ...
      it('should skip external loop connections if loop not completed', () => {
        ...
        mockWorker.connections.push({ source: 'block1', target: 'block3' })
        mockContext.executedBlocks.add('block1')
  ...
  describe('edge cases', () => {
    it('should handle blocks with multiple incoming connections', () => {
      ...
      mockWorker.connections.push({ source: 'router1', target: 'block2' })
    ...
    it('should handle nested loops', () => {
      // Add nested loop
      mockWorker.loops = mockWorker.loops || {}
      mockWorker.loops.loop2 = {
        id: 'loop2',
        nodes: ['loop1', 'block1'],
        iterations: 2,
        loopType: 'for',
      ...
      const loops = Object.entries(mockContext.worker?.loops || {})
    ...
    it('should handle empty worker', () => {
      const emptyWorker: SerializedWorker = {
        version: '1.0',
        blocks: [],
        connections: [],
        loops: {},
      }
      const emptyTracker = new PathTracker(emptyWorker)
