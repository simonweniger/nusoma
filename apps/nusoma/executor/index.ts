...
import type { SerializedBlock, SerializedWorker } from '@/serializer/types'
...
/**
 * Tracks telemetry events for worker execution if telemetry is enabled
 */
function trackWorkerTelemetry(eventName: string, data: Record<string, any>) {
...
/**
 * Core execution engine that runs worker blocks in topological order.
 *
...
export class Executor {
  ...
  private workerInput: any
  private isDebugging = false
  private contextExtensions: any = {}
  private actualWorker: SerializedWorker
  ...
  constructor(
    private workerParam:
      ...
      | {
        ...
        workerInput?: any
        workerVariables?: Record<string, any>
        contextExtensions?: {
          stream?: boolean
    ...
    workerInput?: any,
    private workerVariables: Record<string, any> = {}
  ) {
    // Handle new constructor format with options object
    if (typeof workerParam === 'object' && 'worker' in workerParam) {
      const options = workerParam
      this.actualWorker = options.worker
      this.initialBlockStates = options.currentBlockStates || {}
      this.environmentVariables = options.envVarValues || {}
      this.workerInput = options.workerInput || {}
      this.workerVariables = options.workerVariables || {}
      ...
      this.actualWorker = workerParam

      if (workerInput) {
        this.workerInput = workerInput
        logger.info('[Executor] Using worker input:', JSON.stringify(this.workerInput, null, 2))
      } else {
        this.workerInput = {}
      }
    }
    ...
    this.loopManager = new LoopManager(this.actualWorker.loops || {})
    this.parallelManager = new ParallelManager(this.actualWorker.parallels || {})
    this.resolver = new InputResolver(
      this.actualWorker,
      this.environmentVariables,
      this.workerVariables,
      this.loopManager
    )
    this.pathTracker = new PathTracker(this.actualWorker)
  ...
  /**
   * Executes the worker and returns the result.
   *
   * @param workerId - Unique identifier for the worker execution
   * @returns Execution result containing output, logs, and metadata, or a stream, or combined execution and stream
   */
  async execute(workerId: string): Promise<ExecutionResult | StreamingExecution> {
    ...
    trackWorkerTelemetry('worker_execution_started', {
      workerId,
      blockCount: this.actualWorker.blocks.length,
      connectionCount: this.actualWorker.connections.length,
      startTime: startTime.toISOString(),
    })

    this.validateWorker()

    const context = this.createExecutionContext(workerId, startTime)
    ...
    try {
      ...
      while (hasMoreLayers && iteration < maxIterations) {
        ...
        if (this.isDebugging) {
          ...
          if (nextLayer.length === 0) {
            ...
            // The caller (useWorkerExecution) will handle resumption
            return {
              success: true,
              output: finalOutput,
              metadata: {
                ...
                workerConnections: this.actualWorker.connections.map((conn: any) => ({
                  source: conn.source,
                  target: conn.target,
                })),
              },
      ...
      trackWorkerTelemetry('worker_execution_completed', {
        workerId,
        duration,
        blockCount: this.actualWorker.blocks.length,
      ...
      logger.error('Worker execution failed:', this.sanitizeError(error))

      // Track worker execution failure
      trackWorkerTelemetry('worker_execution_failed', {
        workerId,
        duration: Date.now() - startTime.getTime(),
      ...
      if (isComplete) {
        ...
        return {
          ...
          metadata: {
            ...
            workerConnections: this.actualWorker.connections.map((conn) => ({
              source: conn.source,
              target: conn.target,
            })),
          },
  ...
  /**
   * Validates that the worker meets requirements for execution.
   * Checks for starter block, connections, and loop configurations.
   *
   * @throws Error if worker validation fails
   */
  private validateWorker(): void {
    const starterBlock = this.actualWorker.blocks.find((block) => block.metadata?.id === 'starter')
    if (!starterBlock || !starterBlock.enabled) {
      throw new Error('Worker must have an enabled starter block')
    }

    const incomingToStarter = this.actualWorker.connections.filter(
    ...
    const outgoingFromStarter = this.actualWorker.connections.filter(
    ...
    const blockIds = new Set(this.actualWorker.blocks.map((block) => block.id))
    for (const conn of this.actualWorker.connections) {
    ...
    for (const [loopId, loop] of Object.entries(this.actualWorker.loops || {})) {
   ...
   */
  private createExecutionContext(workerId: string, startTime: Date): ExecutionContext {
    ...
    if (this.actualWorker.loops) {
      for (const loopId of Object.keys(this.actualWorker.loops)) {
    ...
    const starterBlock = this.actualWorker.blocks.find((block) => block.metadata?.id === 'starter')
    if (starterBlock) {
      // Initialize the starter block with the worker input
      try {
        ...
        if (inputFormat && Array.isArray(inputFormat) && inputFormat.length > 0) {
          ...
          for (const field of inputFormat) {
            if (field.name && field.type) {
              // Get the field value from worker input if available
              ...
              const inputValue =
                this.workerInput?.input?.[field.name] !== undefined
                  ? this.workerInput.input[field.name] // Try to get from input.field
                  : this.workerInput?.[field.name] // Fallback to direct field access
          ...
          const rawInputData =
            this.workerInput?.input !== undefined
              ? this.workerInput.input // Use the nested input data
              : this.workerInput // Fallback to direct input
          ...
          if (this.workerInput && typeof this.workerInput === 'object') {
            // Preserve complete workerInput structure to maintain JSON format
            ...
            const starterOutput = {
              response: {
                ...
                message: this.workerInput.input,
                conversationId: this.workerInput.conversationId,
              },
            }
      ...
      const connectedToStarter = this.actualWorker.connections
  ...
  private getNextExecutionLayer(context: ExecutionContext): string[] {
    ...
    for (const block of this.actualWorker.blocks) {
      ...
      for (const [parallelId, parallel] of Object.entries(this.actualWorker.parallels || {})) {
      ...
      if (insideParallel && activeParallels.has(insideParallel)) {
        ...
        for (const virtualBlockId of virtualBlockIds) {
          ...
          const incomingConnections = this.actualWorker.connections.filter(
  ...
  private checkDependencies(
    ...
    const isLoopBlock = incomingConnections.some((conn) => {
      const sourceBlock = this.actualWorker.blocks.find((b) => b.id === conn.source)
    ...
    return incomingConnections.every((conn) => {
      ...
      if (insideParallel !== undefined && iterationIndex !== undefined) {
        ...
        if (
          sourceBlock &&
          this.actualWorker.parallels?.[insideParallel]?.nodes.includes(conn.source)
        ) {
  ...
  private async executeBlock(
    ...
    const block = this.actualWorker.blocks.find((b) => b.id === actualBlockId)
    if (!block) {
    ...
    try {
      ...
      const starterBlock = this.actualWorker.blocks.find((b) => b.metadata?.id === 'starter')
      if (starterBlock) {
      ...
      trackWorkerTelemetry('block_execution_start', {
      ...
      if (
        ...
        trackWorkerTelemetry('block_execution', {
          workerId: context.workerId,
          blockId: block.id,
          virtualBlockId: parallelInfo ? blockId : undefined,
          iterationIndex: parallelInfo?.iterationIndex,
      ...
      trackWorkerTelemetry('block_execution_error', {
        workerId: context.workerId,
        blockId: block.id,
        virtualBlockId: parallelInfo ? blockId : undefined,
        iterationIndex: parallelInfo?.iterationIndex,
  ...
  private activateErrorPath(blockId: string, context: ExecutionContext): boolean {
    ...
    const block = this.actualWorker.blocks.find((b) => b.id === blockId)
    if (
    ...
    const errorConnections = this.actualWorker.connections.filter(
