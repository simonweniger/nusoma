...
describe('InputResolver', () => {
  let sampleWorker: SerializedWorker
  let mockContext: any
  let mockEnvironmentVars: Record<string, string>
  let mockWorkerVars: Record<string, any>
  let resolver: InputResolver

  beforeEach(() => {
    // Set up a sample worker with different types of blocks
    sampleWorker = {
      version: '1.0',
      blocks: [
        {
      ...
      connections: [], // Using connections instead of edges to match SerializedWorker type
      loops: {},
    }
    ...
    // Create resolver
    resolver = new InputResolver(sampleWorker, mockEnvironmentVars, mockWorkerVars)
  })
  ...
  describe('Block Reference Resolution', () => {
    ...
    it('should throw an error for references to disabled blocks', () => {
      ...
      const disabledBlock = sampleWorker.blocks.find((b) => b.id === 'disabled-block')!
  ...
  describe('direct loop references', () => {
    it('should resolve direct loop.currentItem reference without quotes', () => {
      ...
      const resolver = new InputResolver(worker, {})
