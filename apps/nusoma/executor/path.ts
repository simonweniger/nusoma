...
import type { SerializedBlock, SerializedConnection, SerializedWorker } from '@/serializer/types'
import type { BlockState, ExecutionContext } from './types'

const logger = createLogger('PathTracker')

/**
 * Manages the active execution paths in the worker.
 * Tracks which blocks should be executed based on routing decisions.
 */
export class PathTracker {
  constructor(private worker: SerializedWorker) { }
  ...
  private getIncomingConnections(blockId: string): SerializedConnection[] {
    return this.worker.connections.filter((conn) => conn.target === blockId)
  }
  ...
  private getOutgoingConnections(blockId: string): SerializedConnection[] {
    return this.worker.connections.filter((conn) => conn.source === blockId)
  }
  ...
  private getBlock(blockId: string): SerializedBlock | undefined {
    return this.worker.blocks.find((b) => b.id === blockId)
  }
  ...
  private updateConditionPaths(block: SerializedBlock, context: ExecutionContext): void {
    ...
    const targetConnections = this.worker.connections.filter(
  ...
  private getBlockLoops(
    ...
    return Object.entries(context.worker?.loops || {})
