...
export class InputResolver {
  ...
  constructor(
    private worker: SerializedWorker,
    private environmentVariables: Record<string, string>,
    private workerVariables: Record<string, any> = {},
    private loopManager?: LoopManager
  ) {
    // Create maps for efficient lookups
    this.blockById = new Map(worker.blocks.map((block) => [block.id, block]))
    ...
    const starterBlock = worker.blocks.find((block) => block.metadata?.id === 'starter')
    if (starterBlock) {
    ...
    for (const [loopId, loop] of Object.entries(worker.loops || {})) {
   ...
   *
   * @param variable - The variable object from workerVariables
  ...
  /**
   * Resolves worker variable references in a string (<variable.name>).
   *
  ...
  resolveBlockReferences(
    ...
    for (const match of blockMatches) {
      ...
      if (blockRef.toLowerCase() === 'start') {
        ...
        const starterBlock = this.worker.blocks.find((block) => block.metadata?.id === 'starter')
        if (starterBlock) {
  ...
  private findVariableByName(variableName: string): any | undefined {
    const foundVariable = Object.entries(this.workerVariables).find(
  ...
  private resolveLoopReference(
    ...
    const loop = context.worker?.loops[loopId]
    if (!loop) return null
