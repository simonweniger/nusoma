...
/**
 * Timing metadata for worker execution.
 */
export interface ExecutionMetadata {
  startTime?: string // ISO timestamp when worker execution started
  endTime?: string // ISO timestamp when worker execution completed
  duration: number // Duration of worker execution in milliseconds
  pendingBlocks?: string[] // List of block IDs that are pending execution
  isDebugSession?: boolean // Whether the worker is running in debug mode
  context?: ExecutionContext // Runtime context for the worker
  workerConnections?: Array<{ source: string; target: string }> // Connections between worker blocks
}

/**
 * Current state of a block during worker execution.
 */
...
/**
 * Runtime context for worker execution.
 */
export interface ExecutionContext {
  workerId: string // Unique identifier for this worker execution
  ...
  loopItems: Map<string, any> // Tracks current item for forEach loops and parallel distribution - can store any type
  ...
  worker?: SerializedWorker // Reference to the worker being executed
  ...
  edges?: Array<{ source: string; target: string }> // Worker edge connections
...
/**
 * Complete result from executing a worker.
 */
export interface ExecutionResult {
  success: boolean // Whether the worker executed successfully
  output: NormalizedBlockOutput // Final output data from the worker
  error?: string // Error message if execution failed
