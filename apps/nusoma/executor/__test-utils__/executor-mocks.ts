...
export const setupHandlerMocks = () => {
  vi.doMock('../handlers', () => ({
    ...
    WorkerBlockHandler: createMockHandler('worker'),
 ...
 */
export const createMinimalWorker = (): SerializedWorker => ({
  version: '1.0',
  blocks: [
    {
      id: 'starter',
...
export const createWorkerWithCondition = (): SerializedWorker => ({
  version: '1.0',
  blocks: [
    {
      id: 'starter',
...
export const createWorkerWithLoop = (): SerializedWorker => ({
  version: '1.0',
  blocks: [
    {
      id: 'starter',
...
export const createWorkerWithErrorPath = (): SerializedWorker => ({
  version: '1.0',
  blocks: [
    {
      id: 'starter',
...
export const createWorkerWithParallel = (distribution?: any): SerializedWorker => ({
  version: '2.0',
  blocks: [
    {
      id: 'starter',
...
export const createMockContext = (options: MockContextOptions = {}) => {
  const worker = options.worker || createMinimalWorker()

  return {
    workerId: options.workerId || 'test-worker-id',
    ...
    return {
      processParallelIterations:
        ...
        vi.fn().mockImplementation(async (context) => {
          for (const [parallelId, parallel] of Object.entries(context.worker?.parallels || {})) {
...
export const createParallelBlockHandler = vi.fn().mockImplementation(() => {
  return {
    ...
    execute: vi.fn().mockImplementation(async (block, inputs, context) => {
      ...
      const parallel = context.worker?.parallels?.[parallelId]
      ...
      if (!parallelState) {
        ...
        const connections =
          context.worker?.connections.filter(
            (conn: any) =>
...
/**
 * Create a worker with parallel blocks for testing
 */
export const createWorkerWithParallelArray = (
...
/**
 * Create a worker with parallel blocks for object distribution
 */
export const createWorkerWithParallelObject = (
...
export const createWorkerWithResponse = (): SerializedWorker => ({
  version: '1.0',
  blocks: [
    {
      id: 'starter',
