  ...
  async execute(
    ...
    try {
      ...
      const result = await executeTool(block.config.tool, {
        ...processedInputs,
        _context: { workerId: context.workerId },
      })
      ...
      if (!result.success) {
        ...
        if (inputs.url) {
          errorDetails.push(`URL: ${inputs.url}`)
        }
        if (inputs.method) {
          errorDetails.push(`Method: ${inputs.method}`)
        }
        ...
        if (result.output?.status) {
          errorDetails.push(`Status: ${result.output.status}`)
        }
        if (result.output?.statusText) {
          errorDetails.push(`Status text: ${result.output.statusText}`)
        }
      ...
      if (typeof error === 'object' && error !== null) {
        if (!error.toolId) {
          error.toolId = block.config.tool
        }
        if (!error.blockName) {
          error.blockName = block.metadata?.name || 'Unnamed Block'
        }
