...
describe('ConditionBlockHandler', () => {
  ...
  let mockWorker: Partial<SerializedWorker>
  ...
  beforeEach(() => {
    ...
    // Then define worker using the block objects
    mockWorker = {
    ...
    mockPathTracker = new MockPathTracker(mockWorker as SerializedWorker) as Mocked<PathTracker>
    mockResolver = new MockInputResolver(
      mockWorker as SerializedWorker,
      {}
    ...
    // Define mock context *after* worker and blocks are set up
    mockContext = {
      ...
      worker: mockWorker as SerializedWorker,
  ...
  it('should handle condition blocks', () => {
    ...
    const nonCondBlock: SerializedBlock = {
      ...mockBlock,
      metadata: { id: 'other' },
    }
  ...
  it('should execute condition block correctly and select first path', async () => {
    ...
    const result = (await handler.execute(mockBlock, inputs, mockContext)) as {
      response: any
    }
  ...
  it('should resolve references in conditions before evaluation', async () => {
    const conditions = [
      {
        id: 'cond1',
        title: 'if',
        value: '{{source-block-1.response.value}} > 5',
      },
      { id: 'else1', title: 'else', value: '' },
    ]
    const inputs = { conditions: JSON.stringify(conditions) }
    ...
    const _result = (await handler.execute(mockBlock, inputs, mockContext)) as {
      response: any
    }
    ...
    const conditions = [
      {
        id: 'cond1',
        title: 'if',
        value: 'context.nonExistentProperty.doSomething()',
      },
      { id: 'else1', title: 'else', value: '' },
    ]
    const inputs = { conditions: JSON.stringify(conditions) }
  ...
  it('should throw error if target block is missing', async () => {
    ...
    mockContext.worker!.blocks = [mockSourceBlock, mockBlock, mockTargetBlock2]
  ...
  it('should throw error if no condition matches and no else exists', async () => {
    ...
    mockContext.worker!.connections = [
      { source: mockSourceBlock.id, target: mockBlock.id },
      {
        source: mockBlock.id,
        target: mockTargetBlock1.id,
