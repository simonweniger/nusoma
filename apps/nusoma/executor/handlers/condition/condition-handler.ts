...
export class ConditionBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    const sourceBlockId = context.worker?.connections.find(
    ...
    const sourceBlock = context.worker?.blocks.find((b) => b.id === sourceBlockId)
    if (!sourceBlock) {
    ...
    const outgoingConnections = context.worker?.connections.filter(
    ...
    // Find target block
    const targetBlock = context.worker?.blocks.find((b) => b.id === selectedConnection?.target)
    if (!targetBlock) {
