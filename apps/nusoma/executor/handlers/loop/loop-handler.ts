...
export class LoopBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    // Get the loop configuration from the worker
    const loop = context.worker?.loops?.[block.id]
    if (!loop) {
      logger.error(`Loop configuration not found for block ${block.id}`, {
        blockId: block.id,
        availableLoops: Object.keys(context.worker?.loops || {}),
        workerLoops: context.worker?.loops,
      })
