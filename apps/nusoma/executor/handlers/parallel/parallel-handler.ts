...
export class ParallelBlockHandler implements BlockHandler {
  ...
  async execute(
    ...
    // Get the parallel configuration from the worker
    const parallel = context.worker?.parallels?.[block.id]
    if (!parallel) {
    ...
    if (parallelState) {
      ...
      if (allCompleted && !context.completedLoops.has(block.id)) {
        ...
        // Activate the parallel-end-source connection to continue worker
