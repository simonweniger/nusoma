...
import { deleteProjectTaskSchema } from '@/schemas/projects/delete-project-task-schema'

export const deleteProjectTask = authActionClient
  .metadata({ actionName: 'deleteProjectTask' })
  .schema(deleteProjectTaskSchema)
  .action(async ({ parsedInput, ctx }) => {
    ...
    await db.delete(taskTable).where(eq(taskTable.id, parsedInput.id))
