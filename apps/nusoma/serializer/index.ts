...
import type { BlockState, Loop, Parallel } from '@/stores/workers/worker/types'
import type { SerializedBlock, SerializedWorker } from './types'
...
export class Serializer {
  ...
  deserializeWorker(worker: SerializedWorker): {
    blocks: Record<string, BlockState>
    edges: Edge[]
  } {
    ...
    // Deserialize blocks
    worker.blocks.forEach((serializedBlock) => {
      const block = this.deserializeBlock(serializedBlock)
      blocks[block.id] = block
    })

    // Deserialize connections
    worker.connections.forEach((connection) => {
      edges.push({
