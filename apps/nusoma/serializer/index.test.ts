 ...
 * converting between worker state (blocks, edges, loops) and serialized format
 * used by the executor.
 */
import { describe, expect, test, vi } from 'vitest'
import { getProviderFromModel } from '@/providers/utils'
import {
  createAgentWithToolsWorkerState,
  createComplexWorkerState,
  createConditionalWorkerState,
  createInvalidSerializedWorker,
  createInvalidWorkerState,
  createLoopWorkerState,
  createMinimalWorkerState,
  createMissingMetadataWorker,
} from './__test-utils__/test-workers'
...
vi.mock('@/blocks', () => ({
  getBlock: (type: string) => {
    ...
    const mockConfigs: Record<string, any> = {
      starter: {
        name: 'Starter',
        description: 'Start of the worker',
        category: 'flow',
        bgColor: '#4CAF50',
        tools: {
          access: ['starter'],
...
describe('Serializer', () => {
  ...
  describe('serializeWorker', () => {
    test('should serialize a minimal worker correctly', () => {
      const { blocks, edges, loops } = createMinimalWorkerState()
      const serializer = new Serializer()

      const serialized = serializer.serializeWorker(blocks, edges, loops)
    ...
    test('should serialize a conditional worker correctly', () => {
      const { blocks, edges, loops } = createConditionalWorkerState()
      const serializer = new Serializer()
    ...
    test('should serialize a worker with loops correctly', () => {
      const { blocks, edges, loops } = createLoopWorkerState()
      const serializer = new Serializer()
    ...
    test('should serialize a complex worker with multiple block types', () => {
      const { blocks, edges, loops } = createComplexWorkerState()
      const serializer = new Serializer()
    ...
    test('should serialize agent block with custom tools correctly', () => {
      const { blocks, edges, loops } = createAgentWithToolsWorkerState()
      const serializer = new Serializer()
    ...
    test('should handle invalid block types gracefully', () => {
      const { blocks, edges, loops } = createInvalidWorkerState()
      const serializer = new Serializer()

      // Should throw an error when serializing an invalid block type
      expect(() => serializer.serializeWorker(blocks, edges, loops)).toThrow(
   ...
   */
  describe('deserializeWorker', () => {
    test('should deserialize a serialized worker correctly', () => {
      const { blocks, edges, loops } = createMinimalWorkerState()
      const serializer = new Serializer()
      ...
      // Then deserialize
      const deserialized = serializer.deserializeWorker(serialized)
    ...
    test('should deserialize a complex worker with all block types', () => {
      const { blocks, edges, loops } = createComplexWorkerState()
      const serializer = new Serializer()
    ...
    test('should handle serialized worker with invalid block metadata', () => {
      const invalidWorker = createInvalidSerializedWorker() as SerializedWorker
      const serializer = new Serializer()

      // Should throw an error when deserializing an invalid block type
      expect(() => serializer.deserializeWorker(invalidWorker)).toThrow(
        'Invalid block type: non-existent-type'
      )
    })

    test('should handle serialized worker with missing metadata', () => {
      const invalidWorker = createMissingMetadataWorker() as SerializedWorker
      const serializer = new Serializer()
  ...
  describe('round-trip serialization', () => {
    test('should preserve all data through serialization and deserialization', () => {
      ...
      const reserialized = serializer.serializeWorker(
        deserialized.blocks,
        deserialized.edges,
        loops
      )
