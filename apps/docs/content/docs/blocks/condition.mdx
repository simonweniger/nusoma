---
title: Condition
description: Create conditional logic and branching in your workers
---
...
The Condition block allows you to branch your worker execution path based on boolean expressions. It evaluates conditions and routes the worker accordingly, enabling you to create dynamic, responsive workers with different execution paths.
...
The Condition block serves as a decision point in your worker, enabling:
...
<div className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
  ...
  <div className="text-sm text-gray-600 dark:text-gray-400">
    Route workers deterministically without needing an LLM
  </div>
</div>
...
<div className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
  ...
  <div className="text-sm text-gray-600 dark:text-gray-400">
    Create worker paths based on structured data values
  </div>
</div>
  ...
  <div className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
    ...
    <div className="text-sm text-gray-600 dark:text-gray-400">
      Implement conditional programming paradigms in your workers
    </div>
  </div>
</div>
...
<Steps>
  ...
  <Step>
    <strong>Route Worker</strong>: Routes the worker to the appropriate path based on the result
  </Step>
  <Step>
...
Here's an example of how a Condition block might be used in a customer satisfaction worker:
...
Add a catch-all condition (e.g., `true`) as the last condition to handle cases when no other conditions match. This prevents worker execution from getting stuck.
