---
title: Router
description: Route worker execution based on specific conditions or logic
---
...
The Router block is a powerful component in nusoma that intelligently routes worker execution based on content analysis, user input, or predefined conditions. It acts as a decision-making junction in your worker, directing the flow to different paths based on various criteria.
...
The Router block uses LLMs to analyze input content and determine the most appropriate next step in your worker. This allows for:

- Creating dynamic, adaptable workers
...
2. Evaluates the content against the available target blocks in your worker
3. Identifies the most appropriate destination based on the content's intent or requirements
4. Routes the worker execution to the selected block
...
Here's an example of how a Router block might be used in a customer support worker:
