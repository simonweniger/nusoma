---
title: Variables
description: Store and share data across your worker with global variables
---
...
Variables in nusoma act as a global store for data that can be accessed and modified by any block in your worker. They provide a powerful way to share information between different parts of your worker, maintain state, and create more dynamic applications.
...
<Callout type="info">
  Variables allow you to store and share data across your entire worker, making it easy to
  maintain state and create complex, interconnected systems.
</Callout>

## Overview

The Variables feature serves as a central data store for your worker, enabling you to:

<Steps>
  <Step>
    <strong>Store global data</strong>: Create variables that persist throughout worker execution
  </Step>
  <Step>
  ...
  <Step>
    <strong>Maintain worker state</strong>: Keep track of important values as your worker runs
  </Step>
  <Step>
    <strong>Create dynamic workers</strong>: Build more flexible systems that can adapt based on
    stored values
  </Step>
</Steps>
...
Variables can be accessed from any block in your worker using the variable dropdown. Simply:
...
Variables in nusoma can store various types of data:
...
Variables in nusoma have global scope, meaning:

- They are accessible from any block in your worker
- Any block can modify them
- Changes to variables persist throughout worker execution
...
- **Consider Variable Scope**: Remember that variables are global and can be modified by any block. Design your worker with this in mind to prevent unexpected behavior.
- **Initialize Variables Early**: Set up and initialize your variables at the beginning of your worker to ensure they're available when needed.
...
- **Limit Variable Count**: Keep the number of variables manageable. Too many variables can make your worker difficult to understand and maintain.
