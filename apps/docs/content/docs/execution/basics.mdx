...
description: Understanding the fundamental execution flow in nusoma
---
...
When you run a worker in nusoma, the execution engine follows a systematic process to ensure blocks are executed in the correct order and data flows properly between them.

## Execution Flow

The execution of a worker follows these key steps:

<Steps>
  <Step>
    ### Validation Before execution begins, the worker is validated to ensure it has: - An enabled
...
<Step>
  ...
  the worker - Input values from the starter block - Initial state for all blocks - Execution path
    ...
    <Card>
      ...
      <Files>
        <File
          ...
          annotation="Initiates worker execution and provides initial input values. Every worker must have exactly one starter block."
        />
        <File
          name="Router Block"
          icon={<ConnectIcon className="h-4 w-4" />}
...
Run workers on-demand through the nusoma interface by clicking the "Run" button. This is perfect for:

- Testing during development
- One-off tasks
- Workers that need human supervision

### Scheduled Execution

Configure workers to run automatically on a specified schedule:
...
Each worker can be exposed as an API endpoint:

- Get a unique URL for your worker
...
Configure workers to execute in response to external events:

- Set up webhook triggers from third-party services
- Process incoming webhook data as worker input
- Configure webhook security settings
- Support for specialized webhooks (GitHub, Stripe, etc.)

<Callout type="info">
  The execution method you choose depends on your worker's purpose. Manual execution is great for
...
- **Execution Path**: The active path through the worker
- **Routing Decisions**: Records of which paths were selected
- **Environment Variables**: Configuration values for the worker
...
As your worker executes, you can monitor its progress in real-time:
...
These monitoring features help you understand how your worker is executing and identify any issues that arise.
