---
title: Loops
description: Creating iterative processes with loops in nusoma
---
...
Loops are a powerful feature in nusoma that allow you to create iterative processes, implement feedback mechanisms, and build more sophisticated workers.
...
Loops in nusoma allow a group of blocks to execute repeatedly, with each iteration building on the results of the previous one. This enables:
...
<Callout type="info">
  Loops are particularly powerful for AI workers, allowing you to implement techniques like
  chain-of-thought reasoning, recursive refinement, and multi-step problem solving.
</Callout>

## Creating Loops

To create a loop in your worker:
...
<Callout type="warning">
  ...
  iterations is a good starting point for most workers.
</Callout>
...
When a worker with loops executes, the loop manager handles the iteration process:
...
Loops enable powerful worker patterns:
...
To use loops effectively in your workers:
...
When debugging loops in your workers:
...
By mastering loops, you can create much more sophisticated and powerful workers in nusoma.
