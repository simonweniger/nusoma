...
import { updateDocumentComment } from '~/actions/documents/update-document-comment';
import { DeleteDocumentCommentModal } from '~/components/organizations/slug/documents/details/timeline/delete-document-comment-modal';
import { useZodForm } from '~/hooks/use-zod-form';
import { getInitials } from '~/lib/formatters';
import {
  updateDocumentCommentSchema,
  type UpdateDocumentCommentSchema
} from '~/schemas/documents/update-document-comment-schema';
import type { ProfileDto } from '~/types/dtos/profile-dto';
import type { CommentTimelineEventDto } from '~/types/dtos/timeline-event-dto';

type DocumentTimelineCommentProps = {
  profile: ProfileDto;
  event: CommentTimelineEventDto;
};

export function DocumentTimelineComment({
  profile,
  event
}: DocumentTimelineCommentProps): React.JSX.Element {
  const [isEditing, setIsEditing] = React.useState<boolean>(false);
  const form = useZodForm({
    schema: updateDocumentCommentSchema,
    mode: 'all',
    defaultValues: {
      id: event.id,
      text: event.text
  ...
  const handleDelete = (): void => {
    NiceModal.show(DeleteDocumentCommentModal, {
      comment: event
    });
  };
  ...
  const onSubmit: SubmitHandler<UpdateDocumentCommentSchema> = async (
    values
  ) => {
    if (!canSubmit) {
      return;
    }
    const result = await updateDocumentComment(values);
