...
import { documentRecordLabel, documentStageLabel } from '~/lib/labels';
import type { ActivityTimelineEventDto } from '~/types/dtos/timeline-event-dto';

export interface DocumentTimelineActivityProps {
  event: ActivityTimelineEventDto;
}

export function DocumentTimelineActivity({
  event
}: DocumentTimelineActivityProps): React.JSX.Element {
...
const actionTypeToText: Record<ActionType, string> = {
  [ActionType.CREATE]: 'created the document.',
  [ActionType.UPDATE]: 'updated the document.',
  [ActionType.DELETE]: 'deleted the document.'
};
