...
import { addDocumentComment } from '~/actions/documents/add-document-comment';
import { useZodForm } from '~/hooks/use-zod-form';
import { getInitials } from '~/lib/formatters';
import {
  addDocumentCommentSchema,
  type AddDocumentCommentSchema
} from '~/schemas/documents/add-document-comment-schema';
import type { DocumentDto } from '~/types/dtos/document-dto';
import type { ProfileDto } from '~/types/dtos/profile-dto';

export type DocumentTimelineAddCommentCardProps = CardProps & {
  profile: ProfileDto;
...
export function DocumentTimelineAddComment({
  profile,
  document,
  className,
  showComments,
  ...
  ...other
}: DocumentTimelineAddCommentCardProps): React.JSX.Element {
  const methods = useZodForm({
    schema: addDocumentCommentSchema,
    mode: 'all',
    defaultValues: {
  ...
  const onSubmit: SubmitHandler<AddDocumentCommentSchema> = async (
    values
  ): Promise<void> => {
    if (!canSubmit) {
      return;
