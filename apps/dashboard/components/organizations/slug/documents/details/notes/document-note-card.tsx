...
import { DeleteDocumentNoteModal } from '~/components/organizations/slug/documents/details/notes/delete-document-note-modal';
import { EditDocumentNoteModal } from '~/components/organizations/slug/documents/details/notes/edit-document-note-modal';
import { getInitials } from '~/lib/formatters';
import { convertMarkdownToHtml } from '~/lib/markdown';
import type { DocumentNoteDto } from '~/types/dtos/document-note-dto';

type DocumentNoteCardProps = CardProps & {
...
export function DocumentNoteCard({
  ...
  className,
  ...others
}: DocumentNoteCardProps): React.JSX.Element {
  const handleShowEditDocumentNoteModal = (): void => {
    NiceModal.show(EditDocumentNoteModal, { note });
  };
  const handleShowDeleteDocumentNoteModal = (): void => {
    NiceModal.show(DeleteDocumentNoteModal, { note });
  };
  return (
    <Card
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuItem
              ...
              onClick={handleShowEditDocumentNoteModal}
            >
              Edit
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              ...
              onClick={handleShowDeleteDocumentNoteModal}
            >
              Delete
            </DropdownMenuItem>
          </DropdownMenuContent>
