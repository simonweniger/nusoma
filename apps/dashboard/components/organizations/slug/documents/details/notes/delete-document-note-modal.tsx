...
import { deleteDocumentNote } from '~/actions/documents/delete-document-note';
...
export type DeleteDocumentNoteModalProps = NiceModalHocProps & {
  note: DocumentNoteDto;
};

export const DeleteDocumentNoteModal =
  NiceModal.create<DeleteDocumentNoteModalProps>(({ note }) => {
    const modal = useEnhancedModal();
    const mdUp = useMediaQuery(MediaQueries.MdUp, { ssr: false });
    const methods = useZodForm({
      schema: deleteDocumentNoteSchema,
      mode: 'all',
      defaultValues: {
        id: note.id
      }
    ...
    const onSubmit: SubmitHandler<DeleteDocumentSchema> = async (values) => {
      ...
      const result = await deleteDocumentNote(values);
