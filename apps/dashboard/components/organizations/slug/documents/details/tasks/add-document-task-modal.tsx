...
import { addDocumentTask } from '~/actions/documents/add-document-task';
import { useEnhancedModal } from '~/hooks/use-enhanced-modal';
import { useZodForm } from '~/hooks/use-zod-form';
import {
  addDocumentTaskSchema,
  type AddDocumentTaskSchema
} from '~/schemas/documents/add-document-task-schema';

export type AddDocumentTaskModalProps = NiceModalHocProps & {
  documentId: string;
};

export const AddDocumentTaskModal = NiceModal.create<AddDocumentTaskModalProps>(
  ({ documentId }) => {
    const modal = useEnhancedModal();
    const mdUp = useMediaQuery(MediaQueries.MdUp, { ssr: false });
    const methods = useZodForm({
      schema: addDocumentTaskSchema,
      mode: 'onSubmit',
      defaultValues: {
        title: '',
        documentId: documentId,
        dueDate: undefined,
        status: DocumentTaskStatus.OPEN,
        description: ''
      }
    });
    ...
    const onSubmit: SubmitHandler<AddDocumentTaskSchema> = async (values) => {
      if (!canSubmit) {
        return;
      }
      const result = await addDocumentTask(values);
