...
} from '~/schemas/documents/delete-document-schema';
import type { DocumentDto } from '~/types/dtos/document-dto';

export type DeleteDocumentModalProps = NiceModalHocProps & {
  document: DocumentDto;
};

export const DeleteDocumentModal = NiceModal.create<DeleteDocumentModalProps>(
  ({ document }) => {
    ...
    const title = 'Delete this document?';
    const canSubmit =
    ...
    const onSubmit: SubmitHandler<DeleteDocumentSchema> = async (values) => {
      ...
      const result = await deleteDocument(values);
      if (!result?.serverError && !result?.validationErrors) {
        toast.success('Document deleted');
        modal.resolve(true);
        modal.handleClose();
      } else {
        toast.error("Document couldn't be deleted");
      }
    };
    const renderDescription = (
      <>
        The document{' '}
        <strong className="text-foreground font-medium">{document.name}</strong>{' '}
