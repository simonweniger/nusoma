...
import { baseUrl, getDocumentImageUrl } from '@workspace/routes';

import { detectChanges } from '~/actions/documents/_document-event-capture';
...
async function getOrCreateTags(
  ...
  const existingTags = await tx
    ...
    .where(inArray(documentTagTable.text, tagsArray));
  ...
  if (newTagsToInsert.length > 0) {
    const insertedTags = await tx
      ...
      .onConflictDoNothing({ target: [documentTagTable.text] })
...
async function preloadImages(): Promise<ImageCache> {
  ...
  await Promise.all(
    documents.map(async (document) => {
      if (document.image) {
        try {
          const response = await fetch(`${baseUrl.Dashboard}${document.image}`);
          if (response.ok) {
            const mimeType =
              response.headers.get('content-type') ||
              getMimeType(document.image);
            const jsBuffer = await response.arrayBuffer();
            const buffer = Buffer.from(new Uint8Array(jsBuffer));
            imageCache.set(document.image, { buffer, mimeType });
          } else {
            console.warn(
              `Failed to fetch image for ${document.name}: ${response.status} ${response.statusText}`
            );
          }
        } catch (error) {
          console.error(`Error fetching image for ${document.name}:`, error);
        }
      }
    })
  );
...
export async function addExampleData(
  ...
  for (let i = 0; i < documents.length; i += BATCH_SIZE) {
    ...
    await db.transaction(async (tx) => {
      ...
      const documentImageValues: DocumentImageValues[] = [];
      const documentToTagValues: DocumentToDocumentTagValues[] = [];
      const documentActivityValues: DocumentActivityValues[] = [];
      ...
      for (const document of batch) {
        ...
        if (document.image && imageCache.has(document.image)) {
          const { buffer } = imageCache.get(document.image)!;
          imageHash = createHash('sha256').update(buffer).digest('hex');
          imageUrl = getDocumentImageUrl(documentId, imageHash);
        }
        ...
        if (imageHash && document.image && imageCache.has(document.image)) {
          const { buffer, mimeType } = imageCache.get(document.image)!;
          documentImageValues.push({
            documentId,
            data: buffer,
        ...
        document.tags.forEach((tagText) => {
          const tagId = tagsMap.get(tagText);
          if (tagId) {
            documentToTagValues.push({ documentId, documentTagId: tagId });
          }
        });

        documentActivityValues.push({
          documentId,
          ...
          metadata: detectChanges(null, {
            ...
            updatedAt: randomDate,
            createdAt: randomDate,
            tags: document.tags.map((tag) => ({ text: tag }))
          }),
          occurredAt: randomDate
        });

        if (FAVORITE_NAMES.includes(document.name)) {
          favoritesCount++;
          favoriteValues.push({ documentId, userId, order: favoritesCount });
        }
      }

      // 4. Perform Batch Inserts
      if (documentValues.length > 0) {
        await tx.insert(documentTable).values(documentValues);
      }
      if (documentImageValues.length > 0) {
        await tx.insert(documentImageTable).values(documentImageValues);
      }
      if (documentToTagValues.length > 0) {
        await tx.insert(documentToDocumentTagTable).values(documentToTagValues);
      }
      if (documentActivityValues.length > 0) {
        await tx.insert(documentActivityTable).values(documentActivityValues);
      }
