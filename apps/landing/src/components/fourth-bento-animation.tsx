...
export function FourthBentoAnimation({
  ...
  return (
    ...
    >
      <div className='-z-10 absolute inset-0 flex [mask:linear-gradient(180deg,transparent,black_40%,black_40%,transparent)] '>
        <div className=' flex h-full w-1/2 items-start justify-between'>
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className='h-5 w-px bg-primary first:bg-transparent' />
          ))}
        </div>
      ...
      <div
        className='-translate-y-1/2 -translate-x-1/3 absolute top-1/2 left-1/2 grid w-full gap-10'
        ref={containerRef}
      >
        <AnimatePresence>
