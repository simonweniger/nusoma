...
export function useClaudeEvents() {
  ...
  useEffect(() => {
    ...
    const unsubEvent = window.nusoma.onEvent((tabId, event) => {
    ...
    const unsubStatus = window.nusoma.onTabStatusChange((tabId, newStatus, oldStatus) => {
      handleStatusChange(tabId, newStatus, oldStatus)
    })

    const unsubError = window.nusoma.onError((tabId, error) => {
      handleError(tabId, error)
    })

    const unsubSkill = window.nusoma.onSkillStatus((status) => {
      if (status.state === 'failed') {
        console.warn(`[NUSOMA] Skill install failed: ${status.name} — ${status.error}`)
      }
    })
  ...
  // Note: window.nusoma.start() is called via sessionStore.initStaticInfo() in App.tsx.
