...
import type { TabStatus, NormalizedEvent, EnrichedError, Message, TabState, Attachment, CatalogPlugin, PluginStatus, ShortcutSettings } from '../../shared/types'
...
async function playNotificationIfHidden(): Promise<void> {
  ...
  try {
    const visible = await window.nusoma.isVisible()
    if (!visible) {
...
export const useSessionStore = create<State>((set, get) => ({
  ...
  initStaticInfo: async () => {
    try {
      const result = await window.nusoma.start()
      set({
        staticInfo: {
  ...
  loadShortcutSettings: async () => {
    try {
      const settings = await window.nusoma.getShortcutSettings()
  ...
  saveShortcutSettings: async (settings) => {
    ...
    try {
      const result = await window.nusoma.setShortcutSettings(settings)
      set({
        shortcutSettingsSaving: false,
  ...
  setPermissionMode: (mode) => {
    ...
    window.nusoma.setPermissionMode(mode)
  },
  ...
  loadMarketplace: async (forceRefresh) => {
    ...
    try {
      const [catalog, installed] = await Promise.all([
        window.nusoma.fetchMarketplace(forceRefresh),
        window.nusoma.listInstalledPlugins(),
      ])
  ...
  installMarketplacePlugin: async (plugin) => {
    ...
    const result = await window.nusoma.installPlugin(plugin.repo, plugin.installName, plugin.marketplace, plugin.sourcePath, plugin.isSkillMd)
    if (result.ok) {
      set((s) => ({
  ...
  uninstallMarketplacePlugin: async (plugin) => {
    const result = await window.nusoma.uninstallPlugin(plugin.installName)
    if (result.ok) {
      set((s) => ({
  ...
  resumeSession: async (sessionId, title, projectPath) => {
    ...
    try {
      ...
      const history = await window.nusoma.loadSession(sessionId, defaultDir).catch(() => [])
      const messages: Message[] = history.map((m) => ({
        id: nextMsgId(),
        role: m.role as Message['role'],
        content: m.content,
  ...
  respondPermission: (tabId, questionId, optionId) => {
    // Send to backend
    window.nusoma.respondPermission(tabId, questionId, optionId).catch(() => {})
  ...
  sendMessage: (prompt, projectPath) => {
    ...
    window.nusoma.prompt(activeTabId, requestId, {
      prompt: fullPrompt,
