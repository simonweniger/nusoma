...
export function InputBar() {
  ...
  useEffect(() => {
    const unsub = window.nusoma.onWindowShown(() => {
      textareaRef.current?.focus()
    })
    return unsub
  }, [])
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && !showSlashMenu) { window.nusoma.hideWindow() }
  }
    ...
    for (const item of Array.from(items)) {
      if (item.type.startsWith('image/')) {
        ...
        reader.onload = async () => {
          ...
          const attachment = await window.nusoma.pasteImage(dataUrl)
  ...
  const startRecording = useCallback(async () => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const result = await window.nusoma.transcribeAudio(wavBase64)
  ...
  return (
    <div ref={wrapperRef} data-nusoma-ui className="flex flex-col w-full relative">
      {/* Slash command menu */}
      <AnimatePresence>
        {showSlashMenu && (
          <SlashCommandMenu
