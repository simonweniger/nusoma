...
// Appended to Claude's default system prompt so it knows it's running inside NUSOMA.
...
const CLUI_SYSTEM_HINT = [
  ...
  '- When the user asks for images, and public web images are appropriate, proactively find and render them in NUSOMA.',
  ...
  '- Do not ask whether NUSOMA can render images; assume it can.',
...
// safety. This is a deliberate NUSOMA policy choice, not native Claude parity.
...
export class RunManager extends EventEmitter {
  ...
  startRun(requestId: string, options: RunOptions): RunHandle {
    ...
    if (options.hookSettingsPath) {
      // NUSOMA-scoped hook settings: the PreToolUse HTTP hook handles permissions
    ...
    // Always tell Claude it's inside NUSOMA (additive, doesn't replace base prompt)
