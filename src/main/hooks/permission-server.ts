 ...
 *
 * This is a NUSOMA-owned permission broker that approximates Claude Code's
 ...
 *   - Per-run settings files (only NUSOMA-spawned sessions see the hook)
 */
...
export class PermissionServer extends EventEmitter {
  ...
  generateSettingsFile(runToken: string): string {
    ...
    const dir = join(tmpdir(), 'nusoma-hook-config')
    try { mkdirSync(dir, { recursive: true, mode: 0o700 }) } catch {}

    const filePath = join(dir, `nusoma-hook-${runToken}.json`)
