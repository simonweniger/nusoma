...
import { registerPmHandlers } from './pm/ipc-handlers'
...
controlPlane.on('event', (tabId: string, event: NormalizedEvent) => {
  broadcast('nusoma:normalized-event', tabId, event)
})

controlPlane.on('tab-status-change', (tabId: string, newStatus: string, oldStatus: string) => {
  broadcast('nusoma:tab-status-change', tabId, newStatus, oldStatus)
})

controlPlane.on('error', (tabId: string, error: EnrichedError) => {
  broadcast('nusoma:enriched-error', tabId, error)
})
...
ipcMain.handle(IPC.SELECT_DIRECTORY, async () => {
  ...
  // Activation is fine here — user is actively interacting with NUSOMA.
...
ipcMain.handle(IPC.TAKE_SCREENSHOT, async () => {
  ...
  try {
    ...
    const screenshotPath = join(tmpdir(), `nusoma-screenshot-${timestamp}.png`)
...
ipcMain.handle(IPC.PASTE_IMAGE, async (_event, dataUrl: string) => {
  try {
    ...
    const timestamp = Date.now()
    const filePath = join(tmpdir(), `nusoma-paste-${timestamp}.${ext}`)
...
ipcMain.handle(IPC.TRANSCRIBE_AUDIO, async (_event, audioBase64: string) => {
  ...
  const tmpWav = join(tmpdir(), `nusoma-voice-${Date.now()}.wav`)
  try {
...
app.whenReady().then(async () => {
  ...
  tray.setToolTip('nusoma — Claude Code UI')
  tray.on('click', () => toggleWindow('tray click'))
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: 'Show nusoma', click: () => showWindow('tray menu') },
