 ...
 *
 * Respects user-managed skills: if a skill dir exists without .nusoma-version,
...
const VERSION_FILE = '.nusoma-version'
...
function log(msg: string): void {
  ...
  try { appendFileSync(join(homedir(), '.nusoma-debug.log'), line) } catch {}
}
...
async function installGithubSkill(
  ...
  try {
    ...
    // Atomic swap: remove old (if NUSOMA-managed), rename tmp into place
...
async function installSkill(
  ...
  if (existsSync(targetDir)) {
    ...
    if (!meta) {
      // Dir exists but no .nusoma-version — user-managed, don't touch
