...
function TabIndicator({
  ...
  return (
    <BaseTabs.Indicator
      ...
      className={cn(
        "absolute left-0 w-(--active-tab-width) translate-x-(--active-tab-left) -translate-y-1/2 transition-all duration-300 ease-in-out",
        variant === "underline"
          ? "bg-primary top-full z-10 h-px"
          : "bg-accent border-ring/70 top-1/2 -z-1 h-(--active-tab-height) rounded-md border shadow-sm",
        className,
      )}
      {...props}
    />
...
function UnderlinedTabs({
  ...
  return (
    <TabsContext.Provider value={{ variant: "underline" }}>
      <BaseTabs.Root
...
function UnderlinedTabsList({
  ...
  return (
    <BaseTabs.List
      ...
      className={cn(
        "text-muted-foreground relative inline-flex h-12 items-center justify-start gap-x-1",
        className,
      )}
      {...props}
    >
...
function UnderlinedTabsTrigger({
  ...
  return (
    <BaseTabs.Tab
      ...
      className={cn(
        "text-muted-foreground data-selected:text-foreground data-selected:font-medium focus-visible:ring-ring focus-visible:ring-offset-background group relative mx-2 inline-flex h-12 cursor-pointer items-center justify-center gap-1.5 whitespace-nowrap rounded-none border-b-2 border-b-transparent bg-transparent px-2 py-1 pb-3 pt-2 text-sm shadow-none outline-none transition-none focus-visible:ring-2 focus-visible:ring-offset-2 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      {...props}
    />
